...
fn test_private_key_from_mnemonic() {
    ...
    assert_eq!(
        "drt1mlh7q3fcgrjeq0et65vaaxcw6m5ky8jhu296pdxpk9g32zga6uhsy839fr",
    ...
    assert_eq!(
        "drt147877pc2tqv88yfvewhmdfuth845uqpsskky8kaalglzp6unem0qujqxy5",
        address.to_bech32_default().bech32
    );
}
...
fn test_wallet_convert_pem_to_keystore() {
    let _ = create_keystore_file_from_scratch("drt", ALICE_KEYSTORE_PATH_TEST_1);
...
fn test_wallet_convert_keystore_to_pem() {
    let address = create_keystore_file_from_scratch("drt", ALICE_KEYSTORE_PATH_TEST_2);
    ...
    let pem_content =
        Wallet::generate_pem_content("drt", &address, &private_key_str, &public_key_str);
