...
use alloc::vec::Vec;
use numbat_wasm::serde as serde;
...
impl From<u64> for AndesBigUint {
    fn from(item: u64) -> Self {
        unsafe {
            AndesBigUint{ handle: bigIntNew(item as i64) }
        }
    }
}
...
impl AndesBigUint {
    pub fn from_i64(value: i64) -> AndesBigUint {
        unsafe {
            AndesBigUint{ handle: bigIntNew(value) }
        }
    }
}
...
impl Clone for AndesBigUint {
    fn clone(&self) -> Self {
        unsafe {
            ...
            AndesBigUint {handle: clone_handle}
        }        
    }
}
...
impl serde::Serialize for AndesBigUint {
...
impl<'de> serde::Deserialize<'de> for AndesBigUint {
    fn deserialize<D>(deserializer: D) -> Result<AndesBigUint, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        let bytes = deserializer.deserialize_bytes(numbat_wasm::serialize_util::BorrowedBytesVisitor)?;
