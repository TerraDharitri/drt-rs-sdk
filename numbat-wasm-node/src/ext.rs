use numbat_wasm::{Address, ArgBuffer, Box, BoxedBytes, CodeMetadata, Vec, H256};
...
extern {
    ...
    fn getDCDTValue(resultOffset: *const u8) -> usize;
    fn getDCDTTokenName(resultOffset: *const u8) -> usize;
...
impl numbat_wasm::ContractHookApi<AndesBigInt, AndesBigUint> for AndesApiImpl {
	#[inline]
	...
	#[inline]
	fn storage_store_big_uint(&self, key: &[u8], value: &AndesBigUint) {
		unsafe {
	...
	#[inline]
	fn storage_load_big_uint(&self, key: &[u8]) -> AndesBigUint {
		unsafe {
	...
	#[inline]
	fn storage_store_big_int(&self, key: &[u8], value: &AndesBigInt) {
		unsafe {
	...
	#[inline]
	fn storage_load_big_int(&self, key: &[u8]) -> AndesBigInt {
		unsafe {
			let result = bigIntNew(0);
	...
	#[inline]
	fn get_call_value_big_uint(&self) -> AndesBigUint {
		unsafe {
			let result = bigIntNew(0);
	...
	#[inline]
	fn get_dcdt_value_big_uint(&self) -> AndesBigUint {
		unsafe {
			let result = bigIntNew(0);
	...
	fn get_dcdt_token_name(&self) -> Vec<u8> {
		...
		unsafe {
			...
			let name_len = getDCDTTokenName(name.as_mut_ptr());
			name.set_len(name_len as usize);
			name
		}
	}

	fn send_tx(&self, to: &Address, amount: &AndesBigUint, data: &[u8]) {
	...
	fn async_call(&self, to: &Address, amount: &AndesBigUint, data: &[u8]) {
		let amount_bytes32 = amount.to_bytes_be_pad_right(32).unwrap(); // TODO: unwrap panics, remove
		unsafe {
			asyncCall(
				to.as_ref().as_ptr(),
...
impl numbat_wasm::ContractIOApi<AndesBigInt, AndesBigUint> for AndesApiImpl {
	#[inline]
	...
	#[inline]
	fn get_argument_big_uint(&self, arg_id: i32) -> AndesBigUint {
		AndesBigUint {
	...
	#[inline]
	fn get_argument_big_int(&self, arg_id: i32) -> AndesBigInt {
		AndesBigInt {
	...
	#[inline]
	fn finish_big_int(&self, b: &AndesBigInt) {
		unsafe {
	...
	#[inline]
	fn finish_big_uint(&self, b: &AndesBigUint) {
		unsafe {
