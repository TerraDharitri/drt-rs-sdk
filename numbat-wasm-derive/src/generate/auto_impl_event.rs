...
pub fn generate_event_impl(m: &Method, event_identifier: &str) -> proc_macro2::TokenStream {
    ...
    let data_buffer_snippet = if let Some(data_arg) = data_arg {
        ...
        quote! {
            let ___data_buffer___ = numbat_wasm::log_util::serialize_log_data(#data_pat);
        }
    } else {
        quote! {
            let ___data_buffer___ = numbat_wasm::types::ManagedBuffer::<Self::Api>::new();
        }
    };
    ...
    quote! {
        #msig {
            let mut ___topic_accumulator___ = numbat_wasm::log_util::event_topic_accumulator::<Self::Api>(
                #event_identifier_literal,
            );
            #(#topic_push_snippets)*
            #data_buffer_snippet
            numbat_wasm::log_util::write_log(&___topic_accumulator___, &___data_buffer___);
        }
    }
}
...
pub fn generate_legacy_event_impl(m: &Method, event_id_bytes: &[u8]) -> proc_macro2::TokenStream {
    ...
    let topic_conv_snippets: Vec<proc_macro2::TokenStream> = m
        ...
        .map(|arg| {
            let result = if topic_index < nr_args_no_self {
                ...
                let pat = &arg.pat;
                quote! {
                    let data_vec = match numbat_wasm::numbat_codec::top_encode_to_vec_u8(&#pat) {
                        Result::Ok(data_vec) => data_vec,
                        Result::Err(encode_err) => numbat_wasm::api::ErrorApiImpl::signal_error(
    ...
    quote! {
        #msig {
            ...
            #(#topic_conv_snippets)*
            numbat_wasm::api::LogApiImpl::write_legacy_log(
