...
fn call_value_init_snippet(mpm: MethodPayableMetadata) -> proc_macro2::TokenStream {
    match &mpm {
        MethodPayableMetadata::NotPayable => {
            quote! {
                numbat_wasm::io::call_value_init::not_payable::<Self::Api>();
            }
        },
        MethodPayableMetadata::Rewa => {
            quote! {
                numbat_wasm::io::call_value_init::payable_rewa::<Self::Api>();
            }
        },
        MethodPayableMetadata::SingleDcdtToken(token_identifier) => {
            quote! {
                numbat_wasm::io::call_value_init::payable_single_specific_token::<Self::Api>(#token_identifier);
            }
        },
        MethodPayableMetadata::AnyToken => {
            quote! {
                numbat_wasm::io::call_value_init::payable_any::<Self::Api>();
            }
        },
    }
}
...
fn opt_payment_arg_snippet(
    ...
    opt_arg
        ...
        .map(|arg| {
            let pat = &arg.pat;
            quote! {
                let #pat = numbat_wasm::io::call_value_init::#init_fn_name::<Self::Api>();
            }
        })
        .unwrap_or_default()
}
