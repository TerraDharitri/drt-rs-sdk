...
pub fn contract_implementation(
	...
	let wasm_endpoints = quote! {
		...
		pub mod endpoints {
		  use super::*;
		  use numbat_wasm_node::*;

		  fn new_andes_instance() -> #contract_impl_ident<AndesApiImpl, AndesBigInt, AndesBigUint> {
			let api = AndesApiImpl{};
	...
	let function_selector = quote! {
	  ...
	  #api_where
	  {
		...
		fn abi(&self, include_modules: bool) -> numbat_wasm::abi::ContractAbi{
			#abi_body
		}
