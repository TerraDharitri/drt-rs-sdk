	...
	match &arg.ty {
		syn::Type::Reference(type_reference) => {
			...
			if let syn::Type::Slice(slice_type) = &*type_reference.elem {
				...
				quote! {
					numbat_wasm::load_single_arg::<T, BigInt, BigUint, Box<[#slice_elem]>>(self.api.clone(), #arg_index_expr, #arg_name_expr)
				}
			} else {
				...
				if let syn::Type::Path(syn::TypePath { path, .. }) = referenced_type {
					if let Some(ident) = path.get_ident() {
						if ident.to_string() == "str" {
							...
							return quote! {
								numbat_wasm::load_single_arg::<T, BigInt, BigUint, Box<str>>(self.api.clone(), #arg_index_expr, #arg_name_expr)
							};
						}
					}
				}
				...
				quote! {
					numbat_wasm::load_single_arg::<T, BigInt, BigUint, #referenced_type>(self.api.clone(), #arg_index_expr, #arg_name_expr)
				}
			}
		},
		_ => {
			quote! {
				numbat_wasm::load_single_arg::<T, BigInt, BigUint, #arg_ty>(self.api.clone(), #arg_index_expr, #arg_name_expr)
			}
		},
	}
}
...
pub fn generate_load_dyn_multi_arg(
	...
	match &arg.ty {
		syn::Type::Reference(type_reference) => {
			...
			let referenced_type = &*type_reference.elem;
			quote! {
				let #pat: & #referenced_type = &numbat_wasm::load_dyn_multi_arg(#loader_expr, #arg_name_expr, #num_expr);
			}
		},
		_ => {
			quote! {
				let #pat: #arg_ty = numbat_wasm::load_dyn_multi_arg(#loader_expr, #arg_name_expr, #num_expr);
			}
		},
	}
}
