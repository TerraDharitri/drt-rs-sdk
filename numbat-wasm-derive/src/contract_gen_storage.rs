...
fn storage_store_snippet_for_type(type_path_segment: &syn::PathSegment, value_expr: &proc_macro2::TokenStream) -> proc_macro2::TokenStream {
    ...
    match type_str.as_str() {
        ...
        type_name =>
            quote!{
                match numbat_wasm::dcd_serde::to_bytes(#value_expr) {
                    Ok(bytes) => {
    ...
    match type_str.as_str() {
        ...
        type_name => {
            quote!{
                ...
                match numbat_wasm::dcd_serde::from_bytes(value_bytes.as_slice()) {
                    Ok(v) => v,
...
fn generate_key_snippet(key_args: &[MethodArg], identifier: String) -> proc_macro2::TokenStream {
    ...
    if key_args.len() == 0 {
        ...
        quote! {
            let key_bytes = match numbat_wasm::dcd_serde::to_bytes((&#id_literal, #(#arg_pats),* )) {
                Ok(bytes) => bytes,
