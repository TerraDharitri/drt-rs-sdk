...
fn type_payload_size(type_name: &syn::Type) -> proc_macro2::TokenStream {
    quote! {
        <#type_name as numbat_wasm::types::ManagedVecItem>::PAYLOAD_SIZE
    }
}
    ...
    match fields {
        syn::Fields::Named(fields_named) => fields_named
            ...
            .map(|field| {
                ...
                quote! {
                    <#type_name as numbat_wasm::types::ManagedVecItem>::SKIPS_RESERIALIZATION
                }
            })
            .collect(),
        _ => {
    ...
    match fields {
        syn::Fields::Named(fields_named) => fields_named
            ...
            .map(|field| {
                ...
                let type_name = &field.ty;
                quote! {
                    #field_ident: numbat_wasm::types::ManagedVecItem::from_byte_reader(|bytes| {
                        let next_index = index + <#type_name as numbat_wasm::types::ManagedVecItem>::PAYLOAD_SIZE;
    ...
    match fields {
        syn::Fields::Named(fields_named) => fields_named
            ...
            .map(|field| {
                ...
                let type_name = &field.ty;
                quote! {
                    numbat_wasm::types::ManagedVecItem::to_byte_writer(&self.#field_ident, |bytes| {
                        let next_index = index + <#type_name as numbat_wasm::types::ManagedVecItem>::PAYLOAD_SIZE;
...
fn generate_array_init_snippet(ast: &syn::DeriveInput) -> proc_macro2::TokenStream {
    ...
    let self_expr = if ast.generics.params.is_empty() {
        ...
        quote! { #name <numbat_wasm::api::uncallable::UncallableApi> }
    };
    quote! {
        const SELF_PAYLOAD_SIZE: usize = <#self_expr as numbat_wasm::types::ManagedVecItem>::PAYLOAD_SIZE;
...
fn enum_derive(data_enum: &syn::DataEnum, ast: &syn::DeriveInput) -> TokenStream {
    ...
    let gen = quote! {
        impl #impl_generics numbat_wasm::types::ManagedVecItem for #name #ty_generics #where_clause {
