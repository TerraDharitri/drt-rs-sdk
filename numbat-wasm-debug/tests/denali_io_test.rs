...
use numbat_wasm_debug::denali_system::model::Scenario;
...
fn test_scenario_low_to_low_ser_de() {
    let contents = fs::read_to_string("./tests/denali-io/example_raw.scen.json").unwrap();
    let scenario_raw = ScenarioRaw::from_json_str(contents.as_str());

    let serialized = scenario_raw.to_json_string();

    let mut file = File::create("tests/denali-io/serialized_raw.scen.json").unwrap();
...
fn test_scenario_low_to_high_ser_de() {
    let example_raw = fs::read_to_string("./tests/denali-io/example_raw.scen.json").unwrap();
    let example_normalized =
        fs::read_to_string("./tests/denali-io/example_normalized.scen.json").unwrap();
    ...
    let mut file = File::create("tests/denali-io/serialized_normalized_1.scen.json").unwrap();
    file.write_all(serialized.as_bytes()).unwrap();
...
fn test_scenario_high_to_high_ser_de() {
    ...
    let mut file = File::create("tests/denali-io/serialized_normalized_2.scen.json").unwrap();
    file.write_all(serialized.as_bytes()).unwrap();
    assert_eq!(serialized, example_normalized);
}
