...
pub struct ManagedStructWithToken<M: ManagedTypeApi> {
    pub token: numbat_wasm::types::DcdtTokenPayment<M>,
    pub num: u32,
...
fn struct_with_numbers_static() {
    assert_eq!(
        <ManagedStructWithToken<DebugApi> as numbat_wasm::types::ManagedVecItem>::PAYLOAD_SIZE,
        28
    );
    assert!(
        !<ManagedStructWithToken<DebugApi> as numbat_wasm::types::ManagedVecItem>::SKIPS_RESERIALIZATION
    );
}
...
fn struct_to_bytes_writer() {
    ...
    <ManagedStructWithToken<DebugApi> as numbat_wasm::types::ManagedVecItem>::to_byte_writer(
        &s,
        |bytes| {
            arr[0..<ManagedStructWithToken::<DebugApi> as numbat_wasm::types::ManagedVecItem>::PAYLOAD_SIZE].copy_from_slice(bytes);
...
fn struct_from_bytes_reader() {
    ...
    let struct_from_bytes =
        <ManagedStructWithToken<DebugApi> as numbat_wasm::types::ManagedVecItem>::from_byte_reader(
            |bytes| {
