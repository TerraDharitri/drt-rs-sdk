...
use numbat_wasm::types::{ContractCall, DcdtTokenPayment};
use denali::{
...
impl TxCall {
    pub fn to_contract_call(&self) -> ContractCall<DebugApi, ()> {
        ...
        contract_call.rewa_payment = (&self.rewa_value.value).into();
        for dcdt in &self.dcdt_value {
            contract_call.payments.push(DcdtTokenPayment::new(
