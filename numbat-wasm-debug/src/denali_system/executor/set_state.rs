...
fn convert_denali_dcdt_to_world_mock(denali_dcdt: &crate::denali_system::model::Dcdt) -> DcdtData {
    match denali_dcdt {
        crate::denali_system::model::Dcdt::Short(short_dcdt) => {
            let balance = short_dcdt.value.clone();
            let mut dcdt_data = DcdtData::default();
            dcdt_data.instances.add(0, balance);
            dcdt_data
        },
        crate::denali_system::model::Dcdt::Full(full_dcdt) => DcdtData {
...
fn convert_denali_dcdt_instance_to_world_mock(
    denali_dcdt: &crate::denali_system::model::DcdtInstance,
) -> DcdtInstance {
    DcdtInstance {
        nonce: denali_dcdt
            .nonce
