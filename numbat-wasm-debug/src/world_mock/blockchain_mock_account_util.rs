...
impl BlockchainMock {
    pub fn add_account(&mut self, acct: AccountData) {
        ...
        self.add_addr_denali_string(address);
    }

    pub fn add_addr_denali_string(&mut self, address: Address) {
        if self.addr_to_denali_string_map.contains_key(&address) {
            return;
        }

        let addr_pretty = super::address_as_denali_string(&address);
        self.addr_to_denali_string_map.insert(address, addr_pretty);
    }
    ...
    pub fn update_accounts(&mut self, accounts: HashMap<Address, AccountData>) {
        for addr in accounts.keys() {
            self.add_addr_denali_string(addr.clone());
        }
