...
use numbat_wasm::abi::{ContractAbi, EndpointAbi, EndpointMutabilityAbi, InputAbi, OutputAbi};
...
fn write_deploy_method_impl(
    ...
    writeln!(
        file,
        r#"        let result: numbat_interact_snippets::InteractorResult<PlaceholderOutput> = self
            .interactor
            .sc_deploy(
                self.contract
                    .{}({})
...
fn write_payments_declaration(file: &mut File, accepted_tokens: &[&str]) {
    ...
    if first_accepted == "REWA" {
        writeln!(file, "        let rewa_amount = 0u64;").unwrap();
    } else {
        writeln!(
            file,
            "        let token_id = b\"\";
...
fn write_contract_call(file: &mut File, endpoint_abi: &EndpointAbi) {
    let payment_snippet = if endpoint_abi.payable_in_tokens.is_empty() {
        ...
        "\n            .dcdt_transfer(token_id, token_nonce, token_amount)\n"
    };
