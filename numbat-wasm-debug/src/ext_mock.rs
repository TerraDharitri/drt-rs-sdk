use numbat_wasm::{Address, ArgBuffer, BoxedBytes, CodeMetadata, H256};
...
use numbat_wasm::{BigIntApi, BigUintApi};
...
impl fmt::Display for TxInput {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		write!(f, "TxInput {{ func: {}, args: {:?}, call_value: {}, dcdt_token_name: {:?}, dcdt_value: {:?}, from: 0x{}, to: 0x{}\n}}", 
            String::from_utf8(self.func_name.clone()).unwrap(),
            self.args,
            self.call_value,
            self.dcdt_token_name,
            self.dcdt_value,
...
impl numbat_wasm::ContractHookApi<RustBigInt, RustBigUint> for TxContext {
	...
	fn get_dcdt_token_name(&self) -> Vec<u8> {
		self.tx_input_box.dcdt_token_name.clone()
	}
...
impl numbat_wasm::ContractIOApi<RustBigInt, RustBigUint> for TxContext {
