...
pub fn execute_dcdt_nft_update_attriutes(
    tx_input: TxInput,
    tx_cache: TxCache,
) -> (TxResult, BlockchainUpdate) {
    if tx_input.args.len() != 3 {
        let err_result =
            TxResult::from_vm_error("DCDTNFTUpdateAttributes expects 3 arguments".to_string());
    ...
    let dcdt_nft_create_log = TxLog {
        ...
        endpoint: DCDT_NFT_UPDATE_ATTRIBUTES_FUNC_NAME.to_vec(),
        topics: vec![
