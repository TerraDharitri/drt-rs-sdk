...
where
	BigInt: numbat_codec::NestedEncode + 'static,
	BigUint: numbat_codec::NestedEncode + 'static,
{
	...
	fn storage_load_cumulated_validator_reward(&self) -> BigUint {
		self.storage_load_big_uint(storage::protected_keys::NUMBAT_REWARD_KEY)
	}
...
/// The Andes VM provides an implementation directly in the protocol.
...
macro_rules! imports {
	() => {
		...
		use numbat_wasm::{BorrowedMutStorage, Box, BoxedBytes, Queue, VarArgs, Vec};
		use numbat_wasm::{SCError, SCResult, SCResult::Err, SCResult::Ok};
	};
}
...
macro_rules! sc_error {
	($s:expr) => {
		numbat_wasm::SCResult::Err(numbat_wasm::SCError::from($s.as_bytes()))
	};
}
...
macro_rules! sc_try {
	($s:expr) => {
		match $s {
			numbat_wasm::SCResult::Ok(t) => t,
			numbat_wasm::SCResult::Err(e) => {
				return numbat_wasm::SCResult::Err(e);
			},
		}
	};
}
