use numbat_codec::multi_types::{IgnoreValue, OptionalValue};
...
#[cfg(feature = "alloc")]
pub type MultiArgVec<T> = numbat_codec::multi_types::MultiValueVec<T>;
...
#[cfg(feature = "alloc")]
pub type VarArgs<T> = numbat_codec::multi_types::MultiValueVec<T>;
...
#[cfg(feature = "alloc")]
pub type MultiResultVec<T> = numbat_codec::multi_types::MultiValueVec<T>;
...
macro_rules! multi_arg_impls {
    ($(($mval_struct:ident $marg_struct:ident $mres_struct:ident $($n:tt $name:ident)+) )+) => {
        $(
            ...
            pub type $marg_struct<$($name,)+> = numbat_codec::multi_types::$mval_struct<$($name,)+>;

            #[deprecated(
                since = "0.29.0",
                note = "Alias kept for backwards compatibility. Replace with `MultiValue*`"
            )]
            pub type $mres_struct<$($name,)+> = numbat_codec::multi_types::$mval_struct<$($name,)+>;
        )+
    }
}
