...
pub async fn delegation_sc_interact_cli() {
    ...
    match cli.command {
        ...
        Some(delegation_sc_interact_cli::InteractCliCommand::Delegate(args)) => {
            ...
            interact.delegate(&sender, args.rewa).await;
        },
        ...
        Some(delegation_sc_interact_cli::InteractCliCommand::UndelegateFunds(args)) => {
            let sender = Bech32Address::from_bech32_string(args.from.clone());
            interact.undelegate(&sender, args.rewa).await;
        },
...
impl DelegateCallsInteract {
    ...
    pub async fn delegate(&mut self, sender: &Bech32Address, rewa_value: u128) {
        self.interactor
            .tx()
            .from(sender)
            .to(self.state.current_delegation_address())
            .typed(DelegationSCProxy)
            .delegate(BigUint::from(rewa_value)) // Example delegation amount
            .gas(12000000)
            .run()
            .await;
    ...
    pub async fn undelegate(&mut self, sender: &Bech32Address, rewa_value: u128) {
        self.interactor
            .tx()
            .from(sender)
            .to(self.state.current_delegation_address())
            .typed(DelegationSCProxy)
            .undelegate(BigUint::from(rewa_value)) // Example delegation amount
            .gas(12000000)
            .run()
            .await;
