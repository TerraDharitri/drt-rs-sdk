...
use dharitri_sc_snippets::{
    imports::*,
    sdk::{data::dcdt::DcdtBalance, utils::base64_decode},
};
...
impl SysFuncCallsInteract {
    ...
    pub async fn set_roles_for_other(&mut self, token_id: &[u8], roles: Vec<DcdtLocalRole>) {
    ...
    pub async fn set_roles(&mut self, token_id: &str, roles: Vec<DcdtLocalRole>) {
    ...
    pub async fn get_roles(&mut self, token_id: &[u8]) {
        ...
        let result = self
            ...
            .gas(100_000_000u64)
            .typed(DCDTSystemSCProxy)
            .get_special_roles(DcdtTokenIdentifier::from(token_id))
    ...
    pub async fn register_meta_dcdt(
        &mut self,
        issue_cost: RustBigUint,
        token_display_name: &[u8],
        token_ticker: &[u8],
    ...
    pub async fn change_sft_meta_dcdt(&mut self, token_id: &[u8], num_decimals: usize) {
    ...
    pub async fn modify_royalties(&mut self, token_id: &[u8], nonce: u64, new_royalty: u64) {
        ...
        self.interactor
            ...
            .gas(100_000_000u64)
            .typed(UserBuiltinProxy)
            .dcdt_modify_royalties(token_id, nonce, new_royalty)
            .run()
            .await;
    }
    ...
    pub async fn set_new_uris(&mut self, token_id: &[u8], nonce: u64, new_uris: &[String]) {
        ...
        self.interactor
            ...
            .gas(100_000_000u64)
            .typed(UserBuiltinProxy)
            .dcdt_nft_set_new_uris(token_id, nonce, &uris)
            .run()
            .await;
    }

    pub async fn send_dcdt(&mut self, token_id: &[u8], nonce: u64, amount: RustBigUint) {
        ...
        self.interactor
            ...
            .single_dcdt(&token_id.into(), nonce, &amount.into())
            .run()
            .await;
    }
    ...
    pub async fn modify_creator(&mut self, token_id: &[u8], nonce: u64) {
        ...
        self.interactor
            ...
            .gas(100_000_000u64)
            .typed(UserBuiltinProxy)
            .dcdt_nft_modify_creator(token_id, nonce)
            .run()
            .await;
    }
    ...
    pub async fn metadata_recreate<T: TopEncode>(
        ...
        self.interactor
            ...
            .gas(100_000_000u64)
            .typed(UserBuiltinProxy)
            .dcdt_metadata_recreate(token_id, nonce, name, royalties, hash, new_attributes, uris)
            .run()
            .await;
    }
    ...
    pub async fn metadata_update<T: TopEncode>(
        ...
        self.interactor
            ...
            .gas(100_000_000u64)
            .typed(UserBuiltinProxy)
            .dcdt_metadata_update(token_id, nonce, name, royalties, hash, new_attributes, uris)
            .run()
            .await;
    }

    pub async fn get_account_dcdt_tokens(&mut self) -> HashMap<String, DcdtBalance> {
        println!(
            "Retrieving DCDT tokens for {}...",
            self.wallet_address.to_bech32_str()
        );

        self.interactor
            .get_account_dcdt(&self.wallet_address.to_address())
            .await
    }
    ...
    pub async fn check_nft_uris(
        ...
        let dcdt_tokens = self.get_account_dcdt_tokens().await;
