use dharitri_sc_snippets::imports::{DcdtLocalRole, DcdtTokenType, RustBigUint};
...
async fn modify_creator() {
    ...
    // set roles
    interact
        .set_roles(&dynamic_nft_token_id, vec![DcdtLocalRole::NftCreate])
        .await;
    ...
    interact
        .set_roles_for_other(
            dynamic_nft_token_id.as_bytes(),
            vec![DcdtLocalRole::ModifyCreator],
        )
        .await;

    // send to other_address
    interact
        .send_dcdt(dynamic_nft_token_id.as_bytes(), 1u64, 1u64.into())
        .await;
...
async fn get_token_properties() {
    ...
    // issue dynamic NFT
    let dynamic_meta_dcdt_token_id = interact
        .issue_dynamic_token(
            RustBigUint::from(ISSUE_COST),
            b"TESTMETA",
            b"TEST",
    ...
    let token_properties = interact
        .get_token_properties(dynamic_meta_dcdt_token_id.as_bytes())
        .await;

    assert!(token_properties.owner_address == interact.wallet_address.to_address());
    assert!(token_properties.token_name == "TESTMETA");
    assert!(token_properties.token_type == DcdtTokenType::DynamicMeta);
