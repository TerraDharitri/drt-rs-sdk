...
SpaceCraft v2.0.0 brings significant improvements to the DharitrI smart contract development experience, featuring a modernized payments API, enhanced type safety, and updated toolchain compatibility.
...
This major release brings significant improvements to the DharitrI smart contract development experience, featuring a modernized payments API, enhanced type safety, and updated toolchain compatibility.
...
The new payments API introduces a unified approach to handling both REWA and DCDT tokens, treating them equally for consistent and predictable behavior.

#### New Types

- `TokenId`: A more concise and ergonomic replacement for token identifiers with consistent REWA.
    - REWA is always serialized as `REWA-000000`
...
- `Payment`: The new payment type that combines token identifier and amount.
    ...
    - **Consistency**: Treats REWA and DCDTs uniformly
...
Force DCDT multi-transfer even when not strictly required:
...
Should no longer be used, since they rely on the assumption that REWA cannot be transferred together with DCDTs, which is no longer valid, since Spica.

- `RewaOrMultiDcdtPayment` → Use `Payment`
...
- **Unified Token Handling**: REWA and DCDTs treated consistently
