...
The `sc-meta` standalone tool signals version incompatibilities when building contracts. Most importantly it writes a warning to console if dharitri-sc version < `v0.58` and rustc ≥ `v1.87`. This combination is not supported because that version of Rust added LLVM 20, which produces bulk memory opcodes, currently not supported by our VM.
...
2. For each contract it checks for incompatibilities, most notably if dharitri-sc version < `v0.58` and rustc ≥ `v1.87`. It also checks for nightly Rust before dharitri-sc v0.50 and knows about minimum versions for the compiler, per release (although this is somewhat redundant, the compiler performs this check as well). Incompatibility warnings are issued (the process is not stopped).
...
### `TokenIdentifier` renamed to `DcdtTokenIdentifier`

The next release will bring a big revamp of the payment system. For now we just added this rename. `TokenIdentifier` is historical, before we mixed REWA with the DCDTs, and so the name is misleading, since it (also historically) specifically excludes REWA. `DcdtTokenIdentifier` is the appropriate name.
