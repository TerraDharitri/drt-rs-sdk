...
Release v0.55.0 integrates the features of the Spica release into the framework. These mainly revolve around REWA+DCDT multi-transfers.
...
This release attempts to make REWA behave similarly to an DCDT. The purpose in the long run is to unite them in the protocol as well, but for now it suffices to have them dealt with the same way in the SC framework.
...
When sending transactions, `RewaOrDcdtTokenPayment` is the new preferred type, and is properly handled everywhere.

Spica also adds new built-in functions that need framework support: `DCDTSystemSCProxy`: `DCDTModifyRoyalties`, `SDTSetNewURIs`, `DCDTModifyCreator`, `DCDTMetaDataRecreate`, `DCDTMetaDataUpdate`.
