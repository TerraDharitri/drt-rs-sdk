...
3. Before the contract code dump you will find a step by step description of how to create, build and deploy smart contracts on DharitrI
4. All contracts contain a dharitri.json file with the following content:
{
    "language": "rust"
}
...
INSTRUCTIONS FOR CREATING, BUILDING AND DEPLOYING SMART CONTRACTS ON DHARITRI
==============================
...
- src/
  - lib.rs (main contract file, must include #[dharitri_sc::contract] attribute)
- wasm/
- Cargo.toml
...
as modules marked with #[dharitri_sc::module] and import them in the main file).
...
- Use DharitrI SC types (BigUint, ManagedBuffer, etc.)
- Split code into modules using #[dharitri_sc::module] for better organization
...
- wasm/my-contract.drtsc.json (contract metadata)
...
- deploy the contract on devnet using the interactor (recommended for testing, no real REWA needed)
