use crate::{
    chain_core::builtin_func_names::DCDT_MULTI_TRANSFER_FUNC_NAME,
    host::context::TxLog,
...
use crate::{
    builtin_functions::BuiltinFunctionDcdtTransferInfo,
...
use super::{
    ...
    transfer_common::{
        ParsedTransferBuiltinFunCall, RawDcdtTransfer, adjust_call_type,
...
fn build_log(tx_input: &TxInput, call: &ParsedTransferBuiltinFunCall) -> TxLog {
    ...
    let mut data = vec![
        ...
        top_encode_u64(call.raw_dcdt_transfers.len() as u64),
    ];
    push_transfer_bytes(&call.raw_dcdt_transfers, &mut data);
    push_func_name_if_necessary(call_type, &call.func_name, &mut data);

    TxLog {
        address: tx_input.from.clone(),
        endpoint: DCDT_MULTI_TRANSFER_FUNC_NAME.into(),
        topics,
        data,
    }
}
