...
impl<C: VMHooksContext> VMHooksHandler<C> {
    fn perform_transfer_execute_dcdt(
        &mut self,
        to: VMAddress,
        token: Vec<u8>,
    ...
    pub fn transfer_value_execute(
        ...
        let rewa_value = self.context.m_types_lock().bu_get(amount_handle);
        ...
        self.context
            .perform_transfer_execute(recipient, rewa_value, endpoint_name, arg_buffer)
    }

    pub fn multi_transfer_dcdt_nft_execute(
        &mut self,
        to_handle: RawHandle,
        ...
        let (payments, num_bytes_copied) = self
            .context
            .m_types_lock()
            .mb_get_vec_of_dcdt_payments(payments_handle);
        ...
        if payments.len() == 1 {
            ...
            if payment.nonce == 0 {
                self.perform_transfer_execute_dcdt(
                    to,
    ...
    pub fn managed_multi_transfer_dcdt_nft_execute_with_return(
        &mut self,
        to_handle: i32,
        payments_handle: i32,
        gas_limit: u64,
        ...
        self.multi_transfer_dcdt_nft_execute(
            to_handle,
            payments_handle,
            gas_limit,
            function_handle,
    ...
    pub fn async_call_raw(
        ...
        let to = self.context.m_types_lock().mb_to_address(to_handle);
        let rewa_value = self.context.m_types_lock().bu_get(rewa_value_handle);
        let endpoint_name = self
            .context
            .m_types_lock()
            .mb_to_function_name(endpoint_name_handle);
        ...
        self.context
            .perform_async_call(to, rewa_value, endpoint_name, arg_buffer)
    }
    ...
    pub fn deploy_contract(
        ...
        let (new_address, result) =
            self.context
                .perform_deploy(rewa_value, code, code_metadata, arg_buffer)?;
