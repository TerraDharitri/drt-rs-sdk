...
impl ManagedTypeContainer {
    ...
    pub fn mb_get_vec_of_dcdt_payments(
        &self,
        source_handle: RawHandle,
        ...
        assert!(
            data.len() % 16 == 0,
            "malformed ManagedVec<DcdtTokenPayment> data"
        );
    ...
    pub fn mb_set_vec_of_dcdt_payments(
        &mut self,
        dest_handle: RawHandle,
...
pub mod tests {
    ...
    fn test_vec_of_dcdt_payments() {
        ...
        let _ = m_types.mb_set_vec_of_dcdt_payments(handle, transfers.as_slice());
        let (retrieved, _) = m_types.mb_get_vec_of_dcdt_payments(handle);
