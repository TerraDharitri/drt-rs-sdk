...
pub struct TxResult {
    ...
    /// Contains a log generated by DCDT transfer builtin functions,
    /// like `MultiDCDTNFTTransfer` or `DCDTTransfer`.
    ///
    ...
    pub dcdt_transfer_log: Option<TxLog>,
...
impl TxResult {
    ...
    /// Retrieves all logs, including the DCDT transfer log if present.
    pub fn all_logs(&self) -> Vec<&TxLog> {
        let mut all_logs = Vec::new();
        if let Some(dcdt_log) = &self.dcdt_transfer_log {
            all_logs.push(dcdt_log);
        }
    ...
    pub fn append_all_logs(&mut self, source: &mut TxResult) {
        if let Some(dcdt_log) = source.dcdt_transfer_log.take() {
            self.result_logs.push(dcdt_log);
        }
    ...
    pub fn merge_after_sync_call(&mut self, sync_call_result: &TxResult) {
        ...
        if let Some(transfer_log) = &sync_call_result.dcdt_transfer_log {
    ...
    pub fn merge_error(&mut self, error_tx_result: TxResult) {
        ...
        if let Some(transfer_log) = old_value.dcdt_transfer_log {
