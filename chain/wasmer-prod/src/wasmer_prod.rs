...
use dharitri_vm_executor_wasmer::new_traits::{
    WasmerProdExecutor, WasmerProdInstanceState, WasmerProdRuntimeRef,
};

use dharitri_chain_vm::host::{
...
impl InstanceState for WasmerProdInstanceStateAdapter {
    fn get_points_used(&mut self) -> Result<u64, dharitri_vm_executor::ExecutorError> {
    ...
    fn memory_load_to_slice(
        &self,
        mem_ptr: dharitri_vm_executor::MemPtr,
        dest: &mut [u8],
    ) -> Result<(), dharitri_vm_executor::ExecutorError> {
    ...
    fn memory_load_owned(
        &self,
        mem_ptr: dharitri_vm_executor::MemPtr,
        mem_length: dharitri_vm_executor::MemLength,
    ) -> Result<Vec<u8>, dharitri_vm_executor::ExecutorError> {
