use numbat_sdk_drtrs::{data::transaction::Transaction, interactors::wallet::Wallet};
use numbat_wasm_debug::numbat_wasm::types::Address;
use log::debug;
...
impl Interactor {
    pub async fn recall_nonce(&self, address: &Address) -> u64 {
        let drtrs_address = address_h256_to_drtrs(address);
        let account = self
            .proxy
            .get_account(&drtrs_address)
            .await
