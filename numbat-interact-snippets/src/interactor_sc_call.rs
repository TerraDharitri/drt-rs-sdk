use crate::{address_h256_to_drtrs, denali_to_drtrs_address, Interactor, InteractorResult};
use numbat_sdk_drtrs::data::transaction::Transaction;
use numbat_wasm_debug::{
    numbat_wasm::{
        numbat_codec::{multi_types::IgnoreValue, CodecFrom, TopEncodeMulti},
        types::ContractCall,
    },
...
impl Interactor {
    fn tx_call_to_blockchain_tx(&self, tx_call: &TxCall) -> Transaction {
        let contract_call = tx_call.to_contract_call().convert_to_dcdt_transfer_call();
        ...
        Transaction {
            ...
            receiver: address_h256_to_drtrs(&contract_call.to.to_address()),
