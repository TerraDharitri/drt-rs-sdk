    ...
    data_enum
        ...
        .map(|(variant_index, variant)| {
            ...
            if variant.fields.is_empty() {
                ...
                quote! {
                    #name::#variant_ident =>
                        numbat_codec::TopEncode::top_encode_or_handle_err(&#variant_index_u8, output, h),
                }
            } else {
                ...
                quote! {
                    #name::#variant_ident #local_var_declarations => {
                        ...
                        let dest = &mut buffer;
                        numbat_codec::NestedEncode::dep_encode_or_handle_err(&#variant_index_u8, dest, h)?;
...
pub fn top_encode_impl(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let gen = quote! {
        impl #impl_generics numbat_codec::TopEncode for #name #ty_generics #where_clause {
            ...
            where
                O: numbat_codec::TopEncodeOutput,
