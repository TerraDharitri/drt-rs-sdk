...
use crate::{
    ...
    types::{
        ...
        ManagedType, ManagedVec, RewaOrDcdtTokenPayment, TokenIdentifier,
    },
};
    ...
    A: CallTypeApi,
{
    ...
    pub fn transfer_dcdt_execute(
        ...
        self.transfer_dcdt_nft_execute(to, token, 0, value, gas_limit, endpoint_name, arg_buffer);
    }

    #[allow(clippy::too_many_arguments)]
    pub fn transfer_dcdt_nft_execute(
        &self,
        to: &ManagedAddress<A>,
        token: &TokenIdentifier<A>,
        nonce: u64,
        ...
        let mut payments: ManagedVec<A, DcdtTokenPayment<A>> = ManagedVec::new();
    ...
    pub fn multi_dcdt_transfer_execute(
        &self,
        to: &ManagedAddress<A>,
        ...
        A::send_api_impl().multi_transfer_dcdt_nft_execute(
            to.get_handle().get_raw_handle(),
    ...
    /// `multi_transfer_dcdt_nft_execute` doesn't work for a single REWA payment,
    /// so we need a different strategy in this one particular case.
    ///
    /// Returns `true` if single REWA payment was produced.
    fn fallback_to_single_rewa_if_necessary(
        &self,
        to: &ManagedAddress<A>,
        ...
        if payments.is_empty() {
            self.direct_rewa_execute(to, &BigUint::zero(), gas_limit, endpoint_name, arg_buffer);
    ...
    #[deprecated(
        since = "1.20.0",
        note = "Use multi_rewa_or_dcdt_transfer_execute_fallible instead"
    )]
    pub fn multi_rewa_or_dcdt_transfer_execute(
        ...
        if self.fallback_to_single_rewa_if_necessary(
            to,
            payments,
            gas_limit,
            endpoint_name,
    ...
    #[cfg(feature = "barnard")]
    pub fn multi_rewa_or_dcdt_transfer_execute_fallible(
        &self,
        to: &ManagedAddress<A>,
        payments: &ManagedVec<A, RewaOrDcdtTokenPayment<A>>,
        gas_limit: u64,
        ...
        let ret = A::send_api_impl().multi_transfer_dcdt_nft_execute_with_return(
            to.get_handle().get_raw_handle(),
            payments.get_handle().get_raw_handle(),
            gas_limit,
            endpoint_name.get_handle().get_raw_handle(),
    ...
    pub fn multi_rewa_or_dcdt_transfer_execute_fallible(
        ...
        self.multi_rewa_or_dcdt_transfer_execute(
            to,
            payments,
            gas_limit,
            endpoint_name,
