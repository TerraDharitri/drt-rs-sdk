...
use dharitri_chain_core::{
    REWA_000000_TOKEN_IDENTIFIER,
...
use crate::{
    ...
    storage,
    types::{
        BackTransfers, BackTransfersLegacy, BigUint, CodeMetadata, RewaOrDcdtTokenIdentifier,
        RewaOrDcdtTokenPayment, DcdtLocalRoleFlags, DcdtTokenData, DcdtTokenIdentifier,
        DcdtTokenType, ManagedAddress, ManagedBuffer, ManagedByteArray, ManagedRef, ManagedRefMut,
    ...
    pub fn get_sc_balance(&self, token_id: impl AsRef<TokenId<A>>, nonce: u64) -> BigUint<A> {
        ...
        if self.is_native_token(token_id_ref) {
            ...
            self.get_dcdt_balance(
                &self.get_sc_address(),
                unsafe { token_id_ref.as_dcdt_unchecked() },
                nonce,
            )
        }
    }
    ...
    A: ManagedTypeApi + ErrorApi,
{
    pub(crate) fn get_native_token_handle(&self) -> A::ManagedBufferHandle {
        let handle: A::ManagedBufferHandle = use_raw_handle(const_handles::MBUF_REWA_000000);
        A::managed_type_impl()
            .mb_overwrite(handle.clone(), REWA_000000_TOKEN_IDENTIFIER.as_bytes());
        handle
    }

    /// The native token of the given chain. It can currently only return `REWA-000000`.
...
where
    A: BlockchainApi + ManagedTypeApi + ErrorApi,
{
    fn get_dcdt_token_type_raw(
        &self,
        address: &ManagedAddress<A>,
        token_id: &RewaOrDcdtTokenIdentifier<A>,
        nonce: u64,
    ) -> u64 {
        unsafe {
            ...
            A::blockchain_api_impl().managed_get_dcdt_token_type(
                address.get_handle(),
                token_id.get_handle(),
                nonce,
    ...
    pub fn get_dcdt_token_type(
        &self,
        address: &ManagedAddress<A>,
        token_id: &RewaOrDcdtTokenIdentifier<A>,
        nonce: u64,
    ) -> DcdtTokenType {
        DcdtTokenType::from(self.get_dcdt_token_type_raw(address, token_id, nonce) as u8)
    }
    ...
    pub fn get_dcdt_token_data(
        ...
        let token_type = self.get_dcdt_token_type(address, token_id.token_id.as_legacy(), nonce);
    ...
    ///
    /// Covers all cases, including REWA sent via multi-transfer.
    pub fn get_back_transfers(&self) -> BackTransfers<A> {
        unsafe {
            let mut all_bt_vec = ManagedVec::new_uninit();
            let bt_direct_rewa = BigUint::<A>::new_uninit();

            A::blockchain_api_impl().managed_get_back_transfers(
                all_bt_vec.get_raw_handle_unchecked(),
                bt_direct_rewa.get_raw_handle_unchecked(),
            );

            if bt_direct_rewa > 0u64 {
                all_bt_vec.push(RewaOrDcdtTokenPayment::rewa_payment(bt_direct_rewa));
            }
    ...
    #[inline]
    pub fn is_dcdt_paused(&self, token_id: &DcdtTokenIdentifier<A>) -> bool {
        A::blockchain_api_impl().check_dcdt_paused(token_id.get_handle())
    }

    #[inline]
    pub fn is_dcdt_limited_transfer(&self, token_id: &DcdtTokenIdentifier<A>) -> bool {
        A::blockchain_api_impl().check_dcdt_limited_transfer(token_id.get_handle())
    }

    #[inline]
    pub fn get_dcdt_local_roles(&self, token_id: &DcdtTokenIdentifier<A>) -> DcdtLocalRoleFlags {
    ...
    A: BlockchainApi + StorageReadApi + ManagedTypeApi + ErrorApi,
{
    ...
    pub fn token_has_transfer_role(&self, token_identifier: DcdtTokenIdentifier<A>) -> bool {
        // Prepare key
