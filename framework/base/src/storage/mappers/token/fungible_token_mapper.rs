use dharitri_chain_core::types::DcdtLocalRole;
...
use crate::{
    ...
    types::{
        DCDTSystemSCAddress, ManagedRef, Tx,
        system_proxy::{DCDTSystemSCProxy, FungibleTokenProperties},
    },
};
...
use crate::{
    ...
    storage::StorageKey,
    types::{
        BigUint, CallbackClosure, DcdtTokenIdentifier, DcdtTokenPayment, DcdtTokenType,
    ...
    SA: StorageMapperApi + CallTypeApi,
{
    ...
    pub fn send_payment(&self, to: &ManagedAddress<SA>, payment: &DcdtTokenPayment<SA>) {
        Tx::new_tx_from_sc()
            .to(to)
            .single_dcdt(&payment.token_identifier, 0, &payment.amount)
            .transfer();
    }
    ...
    A: StorageAddress<SA>,
{
    ...
    pub fn require_all_same_token(&self, payments: &ManagedVec<SA, DcdtTokenPayment<SA>>) {
        let actual_token_id = self.get_token_id_ref();
        for p in payments {
...
impl<SA> TypeAbiFrom<FungibleTokenMapper<SA>> for DcdtTokenIdentifier<SA> where
    SA: StorageMapperApi + CallTypeApi
{
}
