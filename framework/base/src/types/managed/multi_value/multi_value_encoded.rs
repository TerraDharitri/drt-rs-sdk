use dharitri_sc_codec::MultiValueLength;
use dharitri_sc_codec::multi_types::MultiValue3;
use unwrap_infallible::UnwrapInfallible;

use crate::codec::multi_types::MultiValueVec;
use crate::types::{
    BigUint, RewaOrDcdtTokenIdentifier, RewaOrDcdtTokenPayment, RewaOrDcdtTokenPaymentMultiValue,
    DcdtTokenIdentifier, DcdtTokenPayment, NonZeroBigUint, Payment, PaymentMultiValue, TokenId,
};
use crate::{
...
impl<M> MultiValueEncoded<M, MultiValue3<RewaOrDcdtTokenIdentifier<M>, u64, BigUint<M>>>
where
    M: ManagedTypeApi + ErrorApi,
{
    /// Convenience function to convert a payment multi-argument into a usable structure.
    pub fn convert_payment_multi_triples(self) -> ManagedVec<M, RewaOrDcdtTokenPayment<M>> {
...
impl<M> MultiValueEncoded<M, MultiValue3<DcdtTokenIdentifier<M>, u64, BigUint<M>>>
where
    M: ManagedTypeApi + ErrorApi,
{
    /// Convenience function to convert a payment multi-argument into a usable structure.
    pub fn convert_payment_multi_triples(self) -> ManagedVec<M, DcdtTokenPayment<M>> {
        let mut payments_vec = ManagedVec::new();
...
impl<M> MultiValueEncoded<M, RewaOrDcdtTokenPaymentMultiValue<M>>
where
    M: ManagedTypeApi + ErrorApi,
{
    /// Convenience function to convert a payment multi-argument into a usable structure.
    pub fn convert_payment(self) -> ManagedVec<M, RewaOrDcdtTokenPayment<M>> {
        let mut payments_vec = ManagedVec::new();
    ...
    pub fn from_vec(v: ManagedVec<M, RewaOrDcdtTokenPayment<M>>) -> Self {
