...
use crate::{
    api::ManagedTypeApi,
    types::{
        BigUint, DcdtTokenIdentifier, DcdtTokenPaymentMultiValue, DcdtTokenType, ManagedType,
...
use super::{RewaOrDcdtTokenIdentifier, RewaOrDcdtTokenPayment, MultiRewaOrDcdtPayment};

#[type_abi]
#[derive(TopEncode, NestedEncode, Clone, PartialEq, Eq, Debug)]
pub struct DcdtTokenPayment<M: ManagedTypeApi> {
    pub token_identifier: DcdtTokenIdentifier<M>,
    pub token_nonce: u64,
...
impl<M: ManagedTypeApi> DcdtTokenPayment<M> {
    #[inline]
    pub fn new(
        token_identifier: DcdtTokenIdentifier<M>,
        token_nonce: u64,
        amount: BigUint<M>,
    ) -> Self {
        DcdtTokenPayment {
    ...
    #[inline]
    pub fn into_tuple(self) -> (DcdtTokenIdentifier<M>, u64, BigUint<M>) {
...
impl<M: ManagedTypeApi> From<(DcdtTokenIdentifier<M>, u64, BigUint<M>)> for DcdtTokenPayment<M> {
    #[inline]
    fn from(value: (DcdtTokenIdentifier<M>, u64, BigUint<M>)) -> Self {
...
impl<M: ManagedTypeApi> DcdtTokenPayment<M> {
    ...
    {
        Ok(DcdtTokenPayment {
            token_identifier: DcdtTokenIdentifier::<M>::dep_decode_or_handle_err(input, h)?,
...
pub struct DcdtTokenPaymentRefs<'a, M: ManagedTypeApi> {
    pub token_identifier: &'a DcdtTokenIdentifier<M>,
    pub token_nonce: u64,
...
impl<'a, M: ManagedTypeApi> DcdtTokenPaymentRefs<'a, M> {
    #[inline]
    pub fn new(
        token_identifier: &'a DcdtTokenIdentifier<M>,
        token_nonce: u64,
        amount: &'a BigUint<M>,
    ) -> Self {
...
impl<M: ManagedTypeApi> From<(DcdtTokenIdentifier<M>, u64, BigUint<M>)> for MultiDcdtPayment<M> {
    #[inline]
    fn from(value: (DcdtTokenIdentifier<M>, u64, BigUint<M>)) -> Self {
