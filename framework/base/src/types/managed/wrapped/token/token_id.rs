...
use crate::{
    ...
    types::{
        RewaOrDcdtTokenIdentifier, DcdtTokenIdentifier, LEGACY_REWA_REPRESENTATION, ManagedBuffer,
...
impl<M: ManagedTypeApi> TokenId<M> {
    ...
    /// - Empty buffers to the native token (REWA-000000)
    /// - Legacy "REWA" representation to the native token (REWA-000000)
    /// - All other values are stored as-is
    pub fn new(data: ManagedBuffer<M>) -> Self {
        Self::new_backwards_compatible(data)
    }

    /// Creates a new TokenId with backwards compatibility for legacy REWA representations.
    ///
    /// This method handles the conversion of legacy REWA representations to the current standard:
    /// - Empty buffers are converted to the native token (REWA-000000)
    /// - The legacy "REWA" representation is converted to "REWA-000000"
    ...
    pub fn new_backwards_compatible(data: ManagedBuffer<M>) -> Self {
        const LEGACY_REWA_REPRESENTATION_LEN: usize = LEGACY_REWA_REPRESENTATION.len();
        match data.len() {
            0 => Self::native(),
            LEGACY_REWA_REPRESENTATION_LEN if data == LEGACY_REWA_REPRESENTATION => Self::native(),
    ...
    /// ## Safety
    ///
    /// It does not convert the legacy REWA representation ("REWA" -> "REWA-000000"). Only use if you are certain you are not in this scenario.
    ...
    ///
    /// Returns a TokenId with the value "REWA-000000", which is the current standard
    /// representation for the native REWA token.
    ///
    /// Future developments might make this configurable for custom chains.
    pub fn native() -> Self {
        unsafe { Self::new_unchecked(ManagedBuffer::from(REWA_000000_TOKEN_IDENTIFIER)) }
    }
    ...
    #[inline]
    pub fn into_legacy(self) -> RewaOrDcdtTokenIdentifier<M> {
        RewaOrDcdtTokenIdentifier { token_id: self }
    }

    pub fn as_legacy(&self) -> &RewaOrDcdtTokenIdentifier<M> {
        // safe because of #[repr(transparent)]
        unsafe { core::mem::transmute(self) }
    }

    /// Converts to a specialized DCDT token identifier.
    ///
    /// ## Safety
    ///
    /// Leads to inconsistencies if the token is REWA.
    pub unsafe fn as_dcdt_unchecked(&self) -> &DcdtTokenIdentifier<M> {
        // safe because of #[repr(transparent)]
        unsafe { core::mem::transmute(self) }
    }
    ...
    ///
    /// Leads to inconsistencies if the token is REWA.
    pub unsafe fn into_dcdt_unchecked(self) -> DcdtTokenIdentifier<M> {
        DcdtTokenIdentifier { token_id: self }
    }
    ...
    /// Checks if a token is the native one on the chain. Currently only returns true for `REWA-000000`.
    pub fn is_native(&self) -> bool {
        BlockchainWrapper::<M>::new().is_native_token(self)
    }

    /// Checks the DCDT token identifier for validity.
    ///
...
impl<M: ManagedTypeApi> AsRef<TokenId<M>> for RewaOrDcdtTokenIdentifier<M> {
    fn as_ref(&self) -> &TokenId<M> {
        self.as_token_id()
    }
}
...
impl<M: ManagedTypeApi> From<RewaOrDcdtTokenIdentifier<M>> for TokenId<M> {
    #[inline]
    fn from(token_id: RewaOrDcdtTokenIdentifier<M>) -> Self {
        // RewaOrDcdtTokenIdentifier is also kept in memory with the same representation as TokenId
        // REWA legacy conversion is performed at deserialization, creation and conversion
        token_id.token_id
    }
}
