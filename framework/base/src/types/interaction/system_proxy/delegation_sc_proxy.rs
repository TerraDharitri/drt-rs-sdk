...
use crate::types::{
    BigUint, ManagedAddress, ManagedBuffer, NotPayable, ProxyArg, RewaPayment, Tx, TxEnv, TxFrom,
    ...
    Gas: TxGas<Env>,
{
    ...
    pub fn unjail_nodes<Arg0: ProxyArg<MultiValueVec<BLSKey>> + MultiValueLength>(
        self,
        bls_keys: Arg0,
    ) -> TxTypedCall<Env, From, To, RewaPayment<<Env as TxEnv>::Api>, Gas, ()> {
        let num_keys = bls_keys.multi_value_len();
        self.wrapped_tx
            .raw_call("unJailNodes")
            .rewa(BigUint::from(2500000000000000000u128) * num_keys as u64)
            .argument(&bls_keys)
            .original_result()
    }

    /// The minimum value for creating a new delegation contract is 1 REWA
    pub fn delegate(
        self,
        rewa_value: BigUint<Env::Api>,
    ) -> TxTypedCall<Env, From, To, RewaPayment<<Env as TxEnv>::Api>, Gas, ()> {
        self.wrapped_tx
            .raw_call("delegate")
            .rewa(rewa_value)
    ...
    /// The minimum value for undelegating is 1 REWA
    pub fn undelegate(
        self,
        undelegate_rewa_amount: BigUint<Env::Api>,
    ) -> TxTypedCall<Env, From, To, NotPayable, Gas, ()> {
        self.wrapped_tx
            .raw_call("unDelegate")
            .payment(NotPayable)
            .argument(&undelegate_rewa_amount)
            .original_result()
    }
