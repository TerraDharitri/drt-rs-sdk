    ...
    Env: TxEnv,
{
    ...
    fn perform_transfer_execute_fallible(
        ...
        payments.push(self.to_owned_payment().into_multi_rewa_or_dcdt_payment());

        // using multi-transfer (instead of single DCDT/NFT), because it is the only one that is fallible
    ...
    fn perform_transfer_execute_legacy(
        ...
        if self.token_nonce == 0 {
            // fungible DCDT
            SendRawWrapper::<Env::Api>::new().transfer_dcdt_execute(
                to,
            ...
            SendRawWrapper::<Env::Api>::new().transfer_dcdt_nft_execute(
                to,
                self.token_identifier,
                self.token_nonce,
                self.amount,
