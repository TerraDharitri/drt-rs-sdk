use crate::{
    ...
    types::{BigUint, Rewa, RewaOrDcdtTokenPayment, ManagedAddress, TxFrom, TxToSpecified},
};
    ...
    Env: TxEnv,
{
    ...
    fn perform_transfer_execute_fallible(
        ...
        self.map_ref_rewa_or_dcdt(
            (to, fc),
            |(to, fc), amount| {
                Rewa(amount).perform_transfer_execute_fallible(env, to, gas_limit, fc)
            },
            |(to, fc), dcdt_payment| {
    ...
    fn perform_transfer_execute_legacy(
        ...
        self.map_ref_rewa_or_dcdt(
            (to, fc),
            |(to, fc), amount| Rewa(amount).perform_transfer_execute_legacy(env, to, gas_limit, fc),
            |(to, fc), dcdt_payment| {
                dcdt_payment.perform_transfer_execute_legacy(env, to, gas_limit, fc)
            },
        )
    }
