...
use crate::abi_json::DcdtAttributeAbiJson;

pub fn create_new_dcdt_attr_file(
    json: &DcdtAttributeAbiJson,
    path: impl AsRef<Path>,
    file_name_arg: &str,
) {
    let abi_string = serialize_dcdt_attribute_json(json);
    let mut file_name = file_name_arg.replace(|c: char| !c.is_ascii_alphanumeric(), "_");
    file_name.push_str(".dcdt-abi.json");
...
pub fn serialize_dcdt_attribute_json(json: &DcdtAttributeAbiJson) -> String {
    let buf = Vec::new();
