...
use crate::{
    ...
    drtsc_file_json::{save_drtsc_file_json, DrtscFileJson},
    print_util::*,
...
impl ContractVariant {
    ...
    fn finalize_build(&self, build_args: &BuildArgs, output_path: &Path) {
        ...
        self.pack_drtsc_file(build_args, output_path, &report);
    }
    ...
    fn pack_drtsc_file(&self, build_args: &BuildArgs, output_path: &Path, wasm_report: &WasmReport) {
        let output_wasm_path = output_path.join(self.wasm_output_name(build_args));
        let compiled_bytes = fs::read(output_wasm_path).expect("failed to open compiled contract");
        let output_drtsc_path = output_path.join(self.drtsc_file_output_name(build_args));
        print_pack_drtsc_file(&output_drtsc_path.to_string_lossy());
        ...
        let drtsc_file_json = DrtscFileJson {
            build_info,
            abi,
        ...
        save_drtsc_file_json(&drtsc_file_json, output_drtsc_path);
    }
