...
fn init_rust_types_map() -> HashMap<&'static str, RustTypeString> {
    ...
    m.insert(
        "DcdtTokenIdentifier",
        RustTypeString {
            type_name: "DcdtTokenIdentifier".to_string() + STATIC_API_SUFFIX,
            default_value_expr: "DcdtTokenIdentifier::from_dcdt_bytes(&b\"\"[..])".to_string(),
            contains_custom_types: false,
        },
    );
    m.insert(
    ...
    m.insert(
        "DcdtTokenPayment",
        RustTypeString {
            type_name: "DcdtTokenPayment".to_string() + STATIC_API_SUFFIX,
            default_value_expr: "DcdtTokenPayment::new(
