...
const DRT_STABLE_VERSION: Version = Version::new(0, 50, 0);
const DRT_BULK_MEMORY_FIX_VERSION: Version = Version::new(0, 58, 0);
...
pub fn rustc_version_warning(
    ...
    if framework_version < DRT_STABLE_VERSION
        && rustc_version.version_meta.channel != Channel::Nightly
    {
        return Some(format!("
WARNING! Contract {contract_name} is using dharitri-sc v{framework_version} with Rust {rustc_semver}.
Compiling contracts using dharitri-sc before {DRT_STABLE_VERSION} requires nightly Rust!
Recommended Rust versions: nightly-2023-12-11 or nightly-2024-05-22."));
    }

    if framework_version < DRT_BULK_MEMORY_FIX_VERSION && *rustc_semver >= RUSTC_BULK_MEMORY_VERSION
    {
        return Some(format!("
WARNING! Contract {contract_name} is using dharitri-sc v{framework_version} with Rust {rustc_semver}.
Compiling contracts using dharitri-sc before {DRT_BULK_MEMORY_FIX_VERSION} with Rust v{RUSTC_BULK_MEMORY_VERSION} or newer is not supported!
Either upgrade the framework (to {DRT_BULK_MEMORY_FIX_VERSION} or newer), or downgrade the compiler to v1.86!"));
    }
