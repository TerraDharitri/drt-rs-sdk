...
fn v_0_51_replace_in_files(sc_crate_path: &Path) {
    replace_in_files(
        sc_crate_path,
        "*rs",
        &[Query::simple("dharitri_sc_meta", "dharitri_sc_meta_lib")][..],
    );
}
...
fn v_0_51_prepare_meta(sc_crate_path: &Path) {
    ...
    print_cargo_dep_remove(cargo_toml_path.as_path(), "dharitri-sc-meta");
    let mut meta_value = deps
        .remove("dharitri-sc-meta")
        .expect("dharitri-sc-meta dependency not found in meta crate");
    ...
    deps.insert("dharitri-sc-meta-lib".to_string(), meta_value);
