...
use dharitri_sc::types::{self, Address};
use dharitri_sc_snippets::sdk::{crypto::public_key::PublicKey, wallet::Wallet};
use dharitri_sc_snippets::{hex, imports::Bech32Address};
use std::{
    fs::{self, File},
    io::{self, Read, Write},
};
...
fn write_resulted_pem(hrp: &str, public_key: &str, private_key: &str, outfile: Option<&String>) {
...
fn new(new_args: &WalletNewArgs) {
    ...
    let hrp = new_args.hrp.clone().unwrap_or_else(|| "drt".to_string());
    ...
    println!("Wallet address: {}", address.to_bech32(&hrp));
