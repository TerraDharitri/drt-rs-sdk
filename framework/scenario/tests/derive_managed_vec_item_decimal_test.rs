...
fn struct_with_decimal_static() {
    assert_eq!(
        <ManagedStructWithDecimal<StaticApi> as dharitri_sc::types::ManagedVecItem>::payload_size(
        ),
        24
    );
    assert!(
        !<ManagedStructWithDecimal<StaticApi> as dharitri_sc::types::ManagedVecItem>::SKIPS_RESERIALIZATION
    );
}
...
fn struct_with_decimal_read_write() {
    ...
    let mut payload = <ManagedStructWithDecimal<StaticApi> as dharitri_sc::types::ManagedVecItem>::PAYLOAD::new_buffer();
    <ManagedStructWithDecimal<StaticApi> as dharitri_sc::types::ManagedVecItem>::save_to_payload(
        s.clone(),
        &mut payload,
    );
    let struct_from_bytes =
        <ManagedStructWithDecimal<StaticApi> as dharitri_sc::types::ManagedVecItem>::read_from_payload(
            &payload
        );
