...
pub struct Auction<M: ManagedTypeApi> {
    pub auctioned_token_type: DcdtTokenIdentifier<M>,
    pub auctioned_token_nonce: u64,
    pub nr_auctioned_tokens: BigUint<M>,
    pub auction_type: AuctionType,
    pub payment_token_type: RewaOrDcdtTokenIdentifier<M>,
...
fn struct_3_static() {
    assert_eq!(
        <Auction<StaticApi> as dharitri_sc::types::ManagedVecItem>::payload_size(),
        74
    );
    assert!(!<Auction<StaticApi> as dharitri_sc::types::ManagedVecItem>::SKIPS_RESERIALIZATION);
}
