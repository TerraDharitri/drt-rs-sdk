...
fn enum_static() {
    assert_eq!(
        <EnumWithFields as dharitri_sc::types::ManagedVecItem>::payload_size(),
        9
    );
    assert!(!<EnumWithFields as dharitri_sc::types::ManagedVecItem>::SKIPS_RESERIALIZATION);
}

#[test]
fn enum_to_bytes_writer_variant_1() {
    let mut payload = ManagedVecItemPayloadBuffer::new_buffer();
    <EnumWithFields as dharitri_sc::types::ManagedVecItem>::save_to_payload(
...
fn enum_from_bytes_reader_variant_1() {
    ...
    let enum_from_bytes =
        <EnumWithFields as dharitri_sc::types::ManagedVecItem>::read_from_payload(&payload.into());
