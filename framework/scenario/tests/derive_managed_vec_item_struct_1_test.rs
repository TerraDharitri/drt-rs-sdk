...
fn struct_1_static() {
    assert_eq!(
        <Struct1 as dharitri_sc::types::ManagedVecItem>::payload_size(),
        16
    );
    assert!(<Struct1 as dharitri_sc::types::ManagedVecItem>::SKIPS_RESERIALIZATION);
}
...
fn struct_1_to_bytes_writer() {
    ...
    let mut payload = <Struct1 as dharitri_sc::types::ManagedVecItem>::PAYLOAD::new_buffer();
    <Struct1 as dharitri_sc::types::ManagedVecItem>::save_to_payload(s, &mut payload);
    assert_eq!(
        payload.into_array(),
        [
...
fn struct_1_from_bytes_reader() {
    ...
    let struct_from_bytes =
        <Struct1 as dharitri_sc::types::ManagedVecItem>::read_from_payload(&arr.into());
