...
fn enum_static() {
    assert_eq!(
        <EnumWithFieldsManaged<StaticApi> as dharitri_sc::types::ManagedVecItem>::payload_size(),
        5
    );
    assert!(
        !<EnumWithFieldsManaged<StaticApi> as dharitri_sc::types::ManagedVecItem>::SKIPS_RESERIALIZATION
    );
}
...
fn enum_to_bytes_writer_variant_2() {
    ...
    <EnumWithFieldsManaged<StaticApi> as dharitri_sc::types::ManagedVecItem>::save_to_payload(
...
fn enum_from_bytes_reader_variant_2() {
    unsafe {
        ...
        let enum_from_bytes =
            <EnumWithFieldsManaged<StaticApi> as dharitri_sc::types::ManagedVecItem>::borrow_from_payload(
                &payload.into(),
            )
        ;
        assert_eq!(
