use dharitri_sc::{abi::TypeAbiFrom, types::H256};
use num_traits::Zero;

use crate::{
    api::StaticApi,
    dharitri_sc::{codec::TopEncodeMulti, types::ContractCall},
...
impl ScQueryStep {
    ...
    {
        let (to_str, function, rewa_value_expr, denali_args) = process_contract_call(contract_call);
        assert!(
            rewa_value_expr.value.is_zero(),
            "cannot send REWA value in queries"
        );
