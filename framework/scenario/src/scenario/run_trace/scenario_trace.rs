...
impl ScenarioTrace {
    ...
    pub fn load_scenario_trace<P: AsRef<Path>>(&mut self, file_path: P) {
        let denali_trace_raw = ScenarioRaw::load_from_file(file_path);
        let denali_trace =
            Scenario::interpret_from(denali_trace_raw, &InterpreterContext::default());
        self.scenario_trace = denali_trace;
    }
