...
macro_rules! rust_biguint {
    ($value:expr) => {{ dharitri_sc_scenario::num_bigint::BigUint::from($value as u64) }};
}

#[macro_export]
macro_rules! managed_biguint {
    ($value:expr) => {{ dharitri_sc::types::BigUint::from($value as u64) }};
}

#[macro_export]
macro_rules! managed_buffer {
    ($value:expr) => {{ dharitri_sc::types::ManagedBuffer::new_from_bytes($value) }};
}

#[macro_export]
macro_rules! managed_address {
    ($address:expr) => {{ dharitri_sc::types::ManagedAddress::from_address($address) }};
}

#[macro_export]
macro_rules! managed_token_id {
    ($bytes:expr) => {{ dharitri_sc::types::DcdtTokenIdentifier::from_dcdt_bytes($bytes) }};
}

#[macro_export]
macro_rules! managed_token_id_wrapped {
    ($bytes:expr) => {{
        dharitri_sc::types::RewaOrDcdtTokenIdentifier::dcdt(
            dharitri_sc::types::DcdtTokenIdentifier::from_dcdt_bytes($bytes),
        )
    }};
}
...
macro_rules! token_id {
    ($bytes:expr) => {{ dharitri_sc::types::TokenId::from($bytes) }};
}
