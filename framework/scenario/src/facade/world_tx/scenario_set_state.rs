...
impl ScenarioWorld {
    ...
    pub fn set_dcdt_balance<A, V>(&mut self, address: A, token_id: &[u8], balance: V)
    where
        A: AnnotatedValue<ScenarioTxEnvData, ManagedAddress<StaticApi>>,
        V: AnnotatedValue<ScenarioTxEnvData, BigUint<StaticApi>>,
    {
        ...
        C: AnnotatedValue<ScenarioTxEnvData, ManagedAddress<StaticApi>>,
    {
        ...
        let mut dcdt_attributes = Vec::new();
        let _ = attributes.top_encode(&mut dcdt_attributes);
        let accounts = &mut self.get_mut_state().accounts;
            ...
            if account_address == &address_value.to_address() {
                account.dcdt.set_dcdt_balance(
                    ...
                    DcdtInstanceMetadata {
                        ...
                        attributes: dcdt_attributes.clone(),
    ...
    pub fn set_dcdt_local_roles<A>(&mut self, address: A, token_id: &[u8], roles: &[DcdtLocalRole])
    where
        A: AnnotatedValue<ScenarioTxEnvData, ManagedAddress<StaticApi>>,
    {
        let env = self.new_env_data();
    ...
    ///
    /// Does not trace to Denali, since Denali doesn't have this field yet.
