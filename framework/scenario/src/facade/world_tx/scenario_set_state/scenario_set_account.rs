...
use dharitri_sc::types::{
    AnnotatedValue, BigUint, DcdtTokenIdentifier, ManagedAddress, ManagedBuffer,
};
...
use crate::{
    ...
    scenario_model::{Account, AddressKey, BytesKey, Dcdt, DcdtObject, SetStateStep},
};
...
impl SetStateBuilder<'_, AccountItem> {
    ...
    where
        K: AnnotatedValue<ScenarioTxEnvData, DcdtTokenIdentifier<StaticApi>>,
        V: AnnotatedValue<ScenarioTxEnvData, BigUint<StaticApi>>,
    {
        let env = self.new_env_data();
        ...
        let dcdt_data_ref = self.get_dcdt_data_or_create(&token_id_key);
        dcdt_data_ref.set_balance(0u64, balance_value);
        ...
        T: AnnotatedValue<ScenarioTxEnvData, ManagedBuffer<StaticApi>>,
    {
        ...
        let dcdt_obj_ref = self
            .get_dcdt_data_or_create(&token_id_key)
            .get_mut_dcdt_object();
        dcdt_obj_ref.set_balance(nonce_value.clone(), balance_value);

        dcdt_obj_ref.set_token_attributes(nonce_value, attributes_value);

        self
    }

    #[allow(clippy::too_many_arguments)]
    pub fn dcdt_nft_all_properties<K, N, V, T, C, R, H, U>(
        mut self,
        token_id: K,
        nonce: N,
        balance: V,
        ...
        U: AnnotatedValue<ScenarioTxEnvData, ManagedBuffer<StaticApi>>,
    {
        ...
        dcdt_obj_ref.set_token_all_properties(
            nonce_value,
            balance_value,
    ...
    pub fn dcdt_nft_last_nonce<K, N>(mut self, token_id: K, last_nonce: N) -> Self
    where
        K: AnnotatedValue<ScenarioTxEnvData, DcdtTokenIdentifier<StaticApi>>,
        N: AnnotatedValue<ScenarioTxEnvData, u64>,
    {
        ...
        let dcdt_obj_ref = self
            .get_dcdt_data_or_create(&token_id_key)
            .get_mut_dcdt_object();
        dcdt_obj_ref.set_last_nonce(nonce_value);

        self
    }

    // TODO: Find a better way to pass roles
    pub fn dcdt_roles<K>(mut self, token_id: K, roles: Vec<String>) -> Self
    where
        K: AnnotatedValue<ScenarioTxEnvData, DcdtTokenIdentifier<StaticApi>>,
    {
        let env = self.new_env_data();
    ...
    fn get_dcdt_data_or_create(&mut self, token_id: &BytesKey) -> &mut Dcdt {
        if !self.item.account.dcdt.contains_key(token_id) {
            self.item
                .account
                .dcdt
        ...
        self.item.account.dcdt.get_mut(token_id).unwrap()
    }
    ...
    pub fn storage_denali(mut self, key: &'static str, value: &'static str) -> Self {
