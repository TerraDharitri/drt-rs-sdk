    ...
    match fields {
        syn::Fields::Named(fields_named) => fields_named
            ...
            .map(|field| {
                ...
                quote! {
                    <#type_name as dharitri_sc::types::ManagedVecItem>::SKIPS_RESERIALIZATION
                }
            })
            .collect(),
        _ => {
    ...
    match fields {
        syn::Fields::Named(fields_named) => fields_named
            ...
            .map(|field| {
                ...
                quote! {
                    #field_ident: dharitri_sc::types::managed_vec_item_read_from_payload_index(payload, &mut index),
                }
            })
            .collect(),
        _ => {
    ...
    match fields {
        syn::Fields::Named(fields_named) => fields_named
            ...
            .map(|field| {
                let field_ident = &field.ident;
                quote! {
                    dharitri_sc::types::managed_vec_item_save_to_payload_index(self.#field_ident, payload, &mut index);
                }
            })
            .collect(),
        _ => {
...
fn enum_derive(data_enum: &syn::DataEnum, ast: &syn::DeriveInput) -> TokenStream {
    ...
    for (variant_index, variant) in data_enum.variants.iter().enumerate() {
        ...
        let field_init = if has_field {
            quote! {
                (dharitri_sc::types::managed_vec_item_read_from_payload_index(
                    payload,
                    &mut index
                ))
            }
        ...
        if has_field {
            writer_match_arms.push(quote! {
                #name::#variant_ident (__self_0) => {
                    dharitri_sc::types::managed_vec_item_save_to_payload_index(#variant_index_u8, payload, &mut index);
                    dharitri_sc::types::managed_vec_item_save_to_payload_index(__self_0, payload, &mut index);
                }
            });
        } else {
            writer_match_arms.push(quote! {
    ...
    let result = quote! {
        impl #impl_generics dharitri_sc::types::ManagedVecItem for #name #ty_generics #where_clause {
            type PAYLOAD = <#payload_nested_tuple as dharitri_sc::types::ManagedVecItemEnumPayloadTuple>::EnumPayload;
            const SKIPS_RESERIALIZATION: bool = #skips_reserialization;
            type Ref<'a> = dharitri_sc::types::ManagedVecRef<'a, Self>;

            fn read_from_payload(payload: &Self::PAYLOAD) -> Self {
                let mut index = 0;

                unsafe {
                    let discriminant: u8 = dharitri_sc::types::managed_vec_item_read_from_payload_index(
                        payload,
                        &mut index,
                    );
            ...
            unsafe fn borrow_from_payload<'a>(payload: &Self::PAYLOAD) -> Self::Ref<'a> {
                dharitri_sc::types::ManagedVecRef::new(Self::read_from_payload(payload))
            }
...
fn struct_derive(data_struct: &syn::DataStruct, ast: &syn::DeriveInput) -> TokenStream {
    ...
    let result = quote! {
        impl #impl_generics dharitri_sc::types::ManagedVecItem for #name #ty_generics #where_clause {
            type PAYLOAD = <#payload_nested_tuple as dharitri_sc::types::ManagedVecItemStructPayloadTuple>::StructPayload;
