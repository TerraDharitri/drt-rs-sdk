...
fn enum_derive(data_enum: &syn::DataEnum, ast: &syn::DeriveInput) -> TokenStream {
    ...
    let gen = quote! {
        impl #impl_generics dharitri_sc::types::ManagedVecItem for #name #ty_generics #where_clause {
            type PAYLOAD = dharitri_sc::types::ManagedVecItemPayloadBuffer<1>;
            const SKIPS_RESERIALIZATION: bool = true;
            type Ref<'a> = Self;

            fn read_from_payload(payload: &Self::PAYLOAD) -> Self {
                let discriminant = <u8 as dharitri_sc::types::ManagedVecItem>::read_from_payload(payload);
                match discriminant {
                    #(#reader_match_arms)*
                }
            }
            ...
            fn save_to_payload(self, payload: &mut Self::PAYLOAD) {
                ...
                <u8 as dharitri_sc::types::ManagedVecItem>::save_to_payload(discriminant, payload);
            }
        }
    };
    gen.into()
...
fn struct_derive(data_struct: &syn::DataStruct, ast: &syn::DeriveInput) -> TokenStream {
    ...
    let gen = quote! {
        impl #impl_generics dharitri_sc::types::ManagedVecItem for #name #ty_generics #where_clause {
            type PAYLOAD = <#payload_nested_tuple as dharitri_sc::types::ManagedVecItemNestedTuple>::PAYLOAD;
