            ...
            let ticker = match iter.next() {
                Some(proc_macro2::TokenTree::Literal(literal)) => {
                    ...
                    if clean.chars().next().is_some_and(|s| s == '*') {
                        assert!(
                            clean.len() == 1usize,
                            "attribute needs 1 string argument: \"*\", \"REWA\" or token identifier"
                        );
                    }
                ...
                None => panic!(
                    "argument can not be empty. attribute needs 1 string argument: Replace with #[payable(\"*\")] or #[payable(\"REWA\")"
                ),
            };
