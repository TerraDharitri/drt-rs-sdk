    ...
    format_str_parts.into_iter().map(|part| {
        match part {
            FormatPartType::StaticAscii(ascii_string) => {
                ...
                quote! {
                    dharitri_sc::formatter::FormatBuffer::append_ascii(&mut #accumulator_expr, #str_as_bytes);
                }
            },
            FormatPartType::Display => {
                let arg_expr = tokens_iter.next().unwrap();
                quote! {
                    dharitri_sc::formatter::FormatBuffer::append_display(&mut #accumulator_expr, &#arg_expr);
                }
            },
            FormatPartType::LowerHex => {
                let arg_expr = tokens_iter.next().unwrap();
                quote! {
                    dharitri_sc::formatter::FormatBuffer::append_lower_hex(&mut #accumulator_expr, &#arg_expr);
                }
            },
            FormatPartType::Codec => {
                let arg_expr = tokens_iter.next().unwrap();
                quote! {
                    dharitri_sc::formatter::FormatBuffer::append_codec(&mut #accumulator_expr, &#arg_expr);
                }
            },
            FormatPartType::Bytes => {
                let arg_expr = tokens_iter.next().unwrap();
                quote! {
                    dharitri_sc::formatter::FormatBuffer::append_binary(&mut #accumulator_expr, &#arg_expr);
                }
            },
        }
    }).collect()
