...
pub fn generate_event_impl(m: &Method, event_identifier: &str) -> proc_macro2::TokenStream {
    ...
    let topic_push_snippets: Vec<proc_macro2::TokenStream> = topic_args
        ...
        .map(|arg| {
            ...
            quote! {
                dharitri_sc::log_util::serialize_event_topic(&mut ___topic_accumulator___, #topic_pat);
            }
        })
        .collect();
    let data_buffer_snippet = if let Some(data_arg) = data_arg {
        ...
        quote! {
            let ___data_buffer___ = dharitri_sc::log_util::serialize_log_data(#data_pat);
        }
    } else {
        quote! {
            let ___data_buffer___ = dharitri_sc::types::ManagedBuffer::<Self::Api>::new();
        }
    };
    ...
    quote! {
        #msig {
            let mut ___topic_accumulator___ = dharitri_sc::log_util::event_topic_accumulator::<Self::Api>(
                #event_identifier_literal,
            );
            #(#topic_push_snippets)*
            #data_buffer_snippet
            dharitri_sc::log_util::write_log(&___topic_accumulator___, &___data_buffer___);
        }
    }
}
