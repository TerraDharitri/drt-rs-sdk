...
pub fn generate_proxy_endpoint(m: &Method, endpoint_name: String) -> proc_macro2::TokenStream {
    ...
    if token_count > 0 || nonce_count > 0 || payment_count > 0 {
        ...
        payment_type = quote! { MultiDcdtPayment<Self::Api> };
        payment_init = quote! { .multi_dcdt(#multi_expr.clone_value()) };
    } else {
        payment_type = quote! { () };
        payment_init = quote! {};
    }
    ...
    let return_type = quote! {
        dharitri_sc::types::Tx<
            ...
            dharitri_sc::types::OriginalResultMarker<#original_type>,
        >
    };
...
pub fn generate_proxy_deploy(init_method: &Method) -> proc_macro2::TokenStream {
    ...
    assert!(
        multi_count == 0,
        "No multi DCDT payments allowed in #[init]"
    );
