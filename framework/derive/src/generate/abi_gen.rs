...
fn generate_dcdt_attribute_snippets(contract: &ContractTrait) -> Vec<proc_macro2::TokenStream> {
    contract
        .trait_attributes
        .dcdt_attribute
        .iter()
        .map(|dcdt_attr| {
            let ticker = &dcdt_attr.ticker;
            let ty = &dcdt_attr.ty;
            quote! {
                contract_abi.dcdt_attributes.push(dharitri_sc::abi::DcdtAttributeAbi::new::<#ty>(#ticker));
...
fn generate_abi_method_body(
    ...
    let dcdt_attributes = if !&contract.trait_attributes.dcdt_attribute.is_empty() {
        generate_dcdt_attribute_snippets(contract)
    } else {
        Vec::new()
    };
    ...
    quote! {
        let mut contract_abi = dharitri_sc::abi::ContractAbi::new(
            dharitri_sc::abi::BuildInfoAbi {
                rustc: None,
                contract_crate: dharitri_sc::abi::ContractCrateBuildAbi::new(
                    env!("CARGO_PKG_NAME"),
