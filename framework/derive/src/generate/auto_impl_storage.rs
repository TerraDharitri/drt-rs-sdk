...
fn generate_key_snippet(key_args: &[MethodArgument], identifier: &str) -> proc_macro2::TokenStream {
    ...
    let key_appends: Vec<proc_macro2::TokenStream> = key_args
        ...
        .collect();
    quote! {
        let mut ___key___ = dharitri_sc::storage::StorageKey::<Self::Api>::new(
            &#id_literal[..],
        );
        #(#key_appends)*
    }
    ...
    match m.return_type.clone() {
        ...
        syn::ReturnType::Type(_, _ty) => {
            quote! {
                #msig {
                    #key_snippet
                    dharitri_sc::storage::storage_get(
                        dharitri_sc::types::ManagedRef::new(&___key___),
                    )
                }
            }
        }
...
pub fn generate_setter_impl(m: &Method, identifier: &str) -> proc_macro2::TokenStream {
    ...
    quote! {
        #msig {
            #key_snippet
            dharitri_sc::storage::storage_set(
                dharitri_sc::types::ManagedRef::new(&___key___),
                &#pat);
        }
    }
    ...
    match m.return_type.clone() {
        ...
        syn::ReturnType::Type(_, ty) => {
            quote! {
                #msig {
                    #key_snippet
                    <#ty as dharitri_sc::storage::mappers::StorageMapper<Self::Api>>::new(
                        ___key___
                    )
                }
            }
...
pub fn generate_mapper_from_address_impl(m: &Method, identifier: &str) -> proc_macro2::TokenStream {
    ...
    assert!(
        !m.method_args.is_empty(),
        "mapper from address must have at least one argument, for the value"
    );
    let address_arg = &m.method_args[0];
    let address_pat = address_arg.pat.to_owned();
    ...
    match m.return_type.clone() {
        ...
        syn::ReturnType::Type(_, ty) => {
            quote! {
                #msig {
                    #key_snippet
                    <#ty as dharitri_sc::storage::mappers::StorageMapperFromAddress<Self::Api>>::new_from_address(
                        #address_pat,
                        ___key___
                    )
                }
...
pub fn generate_is_empty_impl(m: &Method, identifier: &str) -> proc_macro2::TokenStream {
    ...
    quote! {
        #msig {
            #key_snippet
            dharitri_sc::storage::storage_get_len(
                dharitri_sc::types::ManagedRef::new(&___key___),
            ) == 0
        }
    }
...
pub fn generate_clear_impl(m: &Method, identifier: &str) -> proc_macro2::TokenStream {
    ...
    quote! {
        #msig {
            #key_snippet
            dharitri_sc::storage::storage_clear(
                dharitri_sc::types::ManagedRef::new(&___key___),
            );
        }
    }
