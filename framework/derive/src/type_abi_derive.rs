...
fn field_snippet(index: usize, field: &syn::Field) -> proc_macro2::TokenStream {
    ...
    quote! {
        field_descriptions.push(dharitri_sc::abi::StructFieldDescription::new(
            &[ #(#field_docs),* ],
            #field_name_str,
            <#field_ty>::type_names(),
        ));
...
pub fn type_abi_derive(input: proc_macro::TokenStream) -> proc_macro2::TokenStream {
    ...
    let type_description_impl = match &ast.data {
        syn::Data::Struct(data_struct) => {
            ...
            quote! {
                fn provide_type_descriptions<TDC: dharitri_sc::abi::TypeDescriptionContainer>(accumulator: &mut TDC) {
                    let type_names = Self::type_names();
                    if !accumulator.contains_type(&type_names.abi) {
                        accumulator.reserve_type_name(type_names.clone());
                        let mut field_descriptions = dharitri_sc::types::heap::Vec::new();
                        #(#struct_field_snippets)*
                        accumulator.insert(
                            type_names.clone(),
                            dharitri_sc::abi::TypeDescription::new(
                                &[ #(#type_docs),* ],
                                type_names,
                                dharitri_sc::abi::TypeContents::Struct(field_descriptions),
                                &[ #(#macro_attributes),* ],
                            ),
                        );
                    }
        ...
        syn::Data::Enum(data_enum) => {
            ...
            let enum_variant_snippets: Vec<proc_macro2::TokenStream> = data_enum
                ...
                .map(|(variant_index, variant)| {
                    ...
                    quote! {
                        ...
                        variant_descriptions.push(dharitri_sc::abi::EnumVariantDescription::new(
                            &[ #(#variant_docs),* ],
                            #variant_name_str,
            ...
            quote! {
                fn provide_type_descriptions<TDC: dharitri_sc::abi::TypeDescriptionContainer>(accumulator: &mut TDC) {
                    ...
                    if !accumulator.contains_type(&type_names.abi) {
                        accumulator.reserve_type_name(type_names.clone());
                        let mut variant_descriptions = dharitri_sc::types::heap::Vec::new();
                        ...
                        accumulator.insert(
                            ...
                            dharitri_sc::abi::TypeDescription::new(
                                &[ #(#type_docs),* ],
                                type_names,
                                dharitri_sc::abi::TypeContents::Enum(variant_descriptions),
                                &[ #(#macro_attributes),* ],
                            ),
                        );
                    }
    ...
    quote! {
        impl #impl_generics dharitri_sc::abi::TypeAbiFrom<Self> for #name #ty_generics #where_clause {}
        impl #impl_generics dharitri_sc::abi::TypeAbiFrom<&Self> for #name #ty_generics #where_clause {}

        impl #impl_generics dharitri_sc::abi::TypeAbi for #name #ty_generics #where_clause {
            type Unmanaged = Self;
