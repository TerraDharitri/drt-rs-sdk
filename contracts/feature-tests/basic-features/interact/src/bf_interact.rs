...
const CODE_EXPR_STORAGE_BYTES: DrtscPath =
    DrtscPath::new("../output/basic-features-storage-bytes.drtsc.json");

const CODE_EXPR: DrtscPath = DrtscPath::new("../output/basic-features.drtsc.json");
const CODE_CRYPTO_EXPR: DrtscPath = DrtscPath::new("../output/basic-features-crypto.drtsc.json");
...
impl BasicFeaturesInteract {
    ...
    pub async fn returns_rewa_decimal(
        &mut self,
        rewa: u64,
    ) -> ManagedDecimal<StaticApi, RewaDecimals> {
    ...
    pub async fn token_has_transfer_role(&mut self, token_id: &str) -> bool {
        self.interactor
            ...
            .gas(50_000_000)
            .typed(basic_features::basic_features_proxy::BasicFeaturesProxy)
            .token_has_transfer_role(TokenIdentifier::from_dcdt_bytes(token_id))
            .returns(ReturnsResult)
            .run()
            .await
    }
