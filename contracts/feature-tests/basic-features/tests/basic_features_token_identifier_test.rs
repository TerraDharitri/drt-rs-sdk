...
fn test_token_identifier_moa() {
    ...
    assert_eq!(MoaOrDcdtTokenIdentifier::moa(), result);
}
...
fn test_token_identifier_is_valid() {
    let bf = basic_features::contract_obj::<StaticApi>();
    let result = bf.token_identifier_is_valid_1(MoaOrDcdtTokenIdentifier::dcdt(
...
fn test_token_identifier_compare() {
    ...
    let dcdt_token_id = MoaOrDcdtTokenIdentifier::dcdt(token_id.clone());
    let wrong_dcdt_token_id =
        MoaOrDcdtTokenIdentifier::dcdt(TokenIdentifier::from(&b"AAA-111111"[..]));
