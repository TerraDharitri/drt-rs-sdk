...
use dharitri_sc_snippets::{hex, imports::*};

const CODE_PATH: DrtscPath = DrtscPath::new("../output/barnard-features.drtsc.json");
...
impl PayableInteract {
    ...
    pub async fn get_dcdt_token_data(
        &mut self,
        ...
        let result_value = self
            ...
            .to(self.state.current_barnard_features_address())
            .typed(barnard_features_proxy::BarnardFeaturesProxy)
            .get_dcdt_token_data(address, TokenIdentifier::from(token_id), nonce)
            .returns(ReturnsResultUnmanaged)
            .run()
            .await;
