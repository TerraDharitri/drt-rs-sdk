...
fn test_managed_values_standalone_consistency() {
    ...
    let foo_token = DcdtTokenIdentifier::<DebugApi>::from_dcdt_bytes(b"FOO-a1a1a1");
    blockchain_wrapper
        .execute_query(&basic_features_wrapper, |_sc| {
            let _bar = DcdtTokenIdentifier::<DebugApi>::from_dcdt_bytes(b"BAR-a1a1a1");
            // 'foo' and '_bar' have the same numerical handle value
            // check that the value of 'foo' is taken from the correct context
            assert_eq!(
                foo_token,
                DcdtTokenIdentifier::<DebugApi>::from_dcdt_bytes(b"FOO-a1a1a1")
            );
        })
        .assert_error(
            err_msg::DEBUG_API_ERR_STATUS,
