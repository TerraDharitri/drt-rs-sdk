...
const CODE_PATH: DrtscPath = DrtscPath::new("../output/payable-features.drtsc.json");
...
pub async fn payable_features_cli() {
    ...
    match &cli.command {
        ...
        Some(payable_interactor_cli::InteractCliCommand::MultiTransferWithOneREWA) => {
            payable_interact
                .check_multi_transfer_only_rewa_transfer()
                .await;
        }
        None => {}
    }
...
impl PayableInteract {
    ...
    pub async fn check_multi_transfer_only_rewa_transfer(&mut self) {
