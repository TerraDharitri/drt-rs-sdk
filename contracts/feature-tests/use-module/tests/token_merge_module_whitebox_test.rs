...
const USE_MODULE_PATH_EXPR: DrtscPath = DrtscPath::new("drtsc:output/use-module.drtsc.json");
...
fn test_token_merge() {
    ...
    world
        ...
        .nonce(1)
        .dcdt_balance(FUNGIBLE_TOKEN_ID_EXPR, FUNGIBLE_AMOUNT)
        .dcdt_nft_all_properties(
            NFT_TOKEN_ID_EXPR,
            FIRST_NFT_NONCE,
            NFT_AMOUNT,
    ...
    world
        ...
        .dcdt_roles(MERGED_TOKEN_ID_EXPR, roles);
    ...
    let nft_transfers = vec![
        TestDcdtTransfer(NFT_TOKEN_ID_EXPR, FIRST_NFT_NONCE, NFT_AMOUNT),
        TestDcdtTransfer(NFT_TOKEN_ID_EXPR, SECOND_NFT_NONCE, NFT_AMOUNT),
    ];
    ...
    world
        ...
        .dcdt_nft_balance_and_attributes(MERGED_TOKEN_ID_EXPR, 1, NFT_AMOUNT, &Empty);

    world
        .check_account(USE_MODULE_ADDRESS_EXPR)
        .dcdt_nft_balance_and_attributes(
            NFT_TOKEN_ID_EXPR,
            FIRST_NFT_NONCE,
            NFT_AMOUNT,
            FIRST_ATTRIBUTES,
    ...
    world
        ...
        .from(USER_ADDRESS_EXPR)
        .to(USE_MODULE_ADDRESS_EXPR)
        .payment(TestDcdtTransfer(MERGED_TOKEN_ID_EXPR, 1, NFT_AMOUNT))
        .whitebox(use_module::contract_obj, |sc| {
    ...
    let dcdt_transfers = vec![
        TestDcdtTransfer(NFT_TOKEN_ID_EXPR, FIRST_NFT_NONCE, NFT_AMOUNT),
        TestDcdtTransfer(FUNGIBLE_TOKEN_ID_EXPR, 0u64, FUNGIBLE_AMOUNT),
    ];
    ...
    world
        .check_account(USER_ADDRESS_EXPR)
        .dcdt_nft_balance_and_attributes(MERGED_TOKEN_ID_EXPR, 2, NFT_AMOUNT, &Empty);

    // merge NFT with an already merged token
    let combined_transfers = vec![
        TestDcdtTransfer(NFT_TOKEN_ID_EXPR, SECOND_NFT_NONCE, NFT_AMOUNT),
        TestDcdtTransfer(MERGED_TOKEN_ID_EXPR, 2u64, NFT_AMOUNT),
    ];
    ...
    world
        .check_account(USER_ADDRESS_EXPR)
        .dcdt_nft_balance_and_attributes(MERGED_TOKEN_ID_EXPR, 3, NFT_AMOUNT, &Empty);

    world
        .tx()
        .from(USER_ADDRESS_EXPR)
        .to(USE_MODULE_ADDRESS_EXPR)
        .payment(TestDcdtTransfer(MERGED_TOKEN_ID_EXPR, 3, NFT_AMOUNT))
        .whitebox(use_module::contract_obj, |sc| {
            let output_tokens = sc.split_tokens_endpoint();
            let expected_output_tokens = vec![
                TestDcdtTransfer(NFT_TOKEN_ID_EXPR, FIRST_NFT_NONCE, NFT_AMOUNT),
                TestDcdtTransfer(FUNGIBLE_TOKEN_ID_EXPR, 0, FUNGIBLE_AMOUNT),
                TestDcdtTransfer(NFT_TOKEN_ID_EXPR, SECOND_NFT_NONCE, NFT_AMOUNT),
            ];
...
fn test_partial_split() {
    ...
    world
        ...
        .from(USER_ADDRESS_EXPR)
        .to(USE_MODULE_ADDRESS_EXPR)
        .payment(TestDcdtTransfer(MERGED_TOKEN_ID_EXPR, 2, NFT_AMOUNT))
        .whitebox(use_module::contract_obj, |sc| {
            let mut tokens_to_remove = ManagedVec::new();
            tokens_to_remove.push(DcdtTokenPayment::new(
                NFT_TOKEN_ID_EXPR.to_token_identifier(),
...
fn test_custom_attributes() {
    ...
    world
        .check_account(USER_ADDRESS_EXPR)
        .dcdt_nft_balance_and_attributes(MERGED_TOKEN_ID_EXPR, 1, NFT_AMOUNT, expected_attributes);
