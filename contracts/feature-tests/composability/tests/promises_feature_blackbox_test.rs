...
const FORWARDER_PATH: DrtscPath = DrtscPath::new("../forwarder/output/forwarder.drtsc.json");
...
impl PromisesFeaturesTestState {
    fn new() -> Self {
        ...
        world
            ...
            .nonce(1)
            .balance(100)
            .dcdt_balance(TOKEN_ID_EXPR, 1000)
            .dcdt_balance(OTHER_TOKEN_ID_EXPR, 1000);
        world
...
fn test_back_transfers() {
    ...
    state
        ...
        .dcdt_balance(TOKEN_ID_EXPR, token_amount);
}
...
fn test_back_transfers_handlers() {
    ...
    let result = state
        ...
        .to(FORWARDER_ADDRESS)
        .typed(forwarder_proxy::ForwarderProxy)
        .forward_sync_accept_funds_rh_single_dcdt(VAULT_ADDRESS)
        .single_dcdt(&TOKEN_ID_EXPR.to_token_identifier(), 0u64, &token_amount)
        .returns(ReturnsResult)
        .run();
    ...
    let mut multi_transfer = ManagedVec::<StaticApi, DcdtTokenPayment<StaticApi>>::new();
    multi_transfer.push(DcdtTokenPayment::new(
    ...
    let result = state
        ...
        .to(FORWARDER_ADDRESS)
        .typed(forwarder_proxy::ForwarderProxy)
        .forward_sync_accept_funds_rh_multi_dcdt(VAULT_ADDRESS)
