use dharitri_sc::types::{TestAddress, TestSCAddress, TestTokenIdentifier};

use dharitri_sc_scenario::{imports::*, ScenarioTxRun, ScenarioWorld};

use promises_features::*;

const PROMISES_FEATURES_CODE_PATH: DrtscPath = DrtscPath::new("output/promises-features.drtsc.json");
...
impl PromisesFeaturesTestState {
    fn new() -> Self {
        ...
        world
            ...
            .nonce(1)
            .dcdt_balance(TEST_TOKEN_ID, 1000u64)
...
fn promises_transfer_test() {
    ...
    state
        ...
        .dcdt_balance(TEST_TOKEN_ID, initial_amount.clone());

    state
        .world
        .check_account(USER_ADDRESS)
        .dcdt_balance(TEST_TOKEN_ID, 0u64);
    ...
    state
        ...
        .single_dcdt(&(TEST_TOKEN_ID.into()), 0, &transfer_amount)
    ...
    state
        .world
        .check_account(OWNER_ADDRESS)
        .dcdt_balance(TEST_TOKEN_ID, initial_amount - transfer_amount.clone());

    state
        .world
        .check_account(USER_ADDRESS)
        .dcdt_balance(TEST_TOKEN_ID, transfer_amount.clone());
}
