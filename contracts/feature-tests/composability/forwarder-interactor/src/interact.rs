...
pub async fn forwarder_cli() {
    ...
    match cmd.as_str() {
        ...
        "forward_sync_accept_funds_rh_multi_dcdt" => {
            interact.forward_sync_accept_funds_rh_multi_dcdt().await
        }
        ...
        "getFungibleDcdtBalance" => interact.get_fungible_dcdt_balance().await,
        "getCurrentNftNonce" => interact.get_current_nft_nonce().await,
        "send_dcdt" => interact.send_dcdt().await,
        "send_dcdt_with_fees" => interact.send_dcdt_with_fees().await,
        "send_dcdt_twice" => interact.send_dcdt_twice().await,
        "send_dcdt_direct_multi_transfer" => interact.send_dcdt_direct_multi_transfer().await,
        ...
        "get_dcdt_local_roles" => interact.get_dcdt_local_roles().await,
        "get_dcdt_token_data" => interact.get_dcdt_token_data().await,
        "is_dcdt_frozen" => interact.is_dcdt_frozen().await,
        "is_dcdt_paused" => interact.is_dcdt_paused().await,
        "is_dcdt_limited_transfer" => interact.is_dcdt_limited_transfer().await,
...
impl ContractInteract {
    ...
    pub async fn echo_arguments_sync(&mut self) {
        ...
        let response = self
            ...
            .payment((
                DcdtTokenIdentifier::from(token_id.as_str()),
                token_nonce,
                token_amount,
            ))
            .returns(ReturnsResultUnmanaged)
    ...
    pub async fn forward_sync_accept_funds_rh_single_dcdt(&mut self) {
        let token_id = String::new();
        let token_nonce = 0u64;
        let token_amount = BigUint::<StaticApi>::from(0u128);
        ...
        let response = self
            ...
            .gas(80_000_000u64)
            .typed(proxy::ForwarderProxy)
            .forward_sync_accept_funds_rh_single_dcdt(to)
            .payment((
                DcdtTokenIdentifier::from(token_id.as_str()),
                token_nonce,
                token_amount,
    ...
    pub async fn forward_sync_accept_funds_rh_multi_dcdt(&mut self) {
        let token_id = String::new();
        let token_nonce = 0u64;
        let token_amount = BigUint::<StaticApi>::from(0u128);
        ...
        let response = self
            ...
            .gas(80_000_000u64)
            .typed(proxy::ForwarderProxy)
            .forward_sync_accept_funds_rh_multi_dcdt(to)
            .payment((
                DcdtTokenIdentifier::from(token_id.as_str()),
                token_nonce,
                token_amount,
    ...
    pub async fn forward_sync_retrieve_funds_with_accept_func(&mut self) {
        ...
        let to = Address::zero();
        let token = DcdtTokenIdentifier::from_dcdt_bytes(&b""[..]);
        let amount = BigUint::<StaticApi>::from(0u128);
    ...
    pub async fn get_fungible_dcdt_balance(&mut self) {
        let token_identifier = DcdtTokenIdentifier::from_dcdt_bytes(&b""[..]);
        ...
        let result_value = self
            ...
            .to(self.state.current_address())
            .typed(proxy::ForwarderProxy)
            .get_fungible_dcdt_balance(token_identifier)
            .returns(ReturnsResultUnmanaged)
            .run()
            .await;
    ...
    pub async fn send_dcdt(&mut self) {
        let to = Address::zero();
        let token_id = DcdtTokenIdentifier::from_dcdt_bytes(&b""[..]);
        let amount = BigUint::<StaticApi>::from(0u128);
    ...
    pub async fn send_dcdt_with_fees(&mut self) {
        let token_id = String::new();
        let token_nonce = 0u64;
        let token_amount = BigUint::<StaticApi>::from(0u128);
        ...
        let response = self
            ...
            .gas(80_000_000u64)
            .typed(proxy::ForwarderProxy)
            .send_dcdt_with_fees(to, percentage_fees)
            .payment((
                DcdtTokenIdentifier::from(token_id.as_str()),
                token_nonce,
                token_amount,
    ...
    pub async fn send_dcdt_twice(&mut self) {
        let to = Address::zero();
        let token_id = DcdtTokenIdentifier::from_dcdt_bytes(&b""[..]);
        ...
        let response = self
            ...
            .gas(80_000_000u64)
            .typed(proxy::ForwarderProxy)
            .send_dcdt_twice(to, token_id, amount_first_time, amount_second_time)
            .returns(ReturnsResultUnmanaged)
            .run()
            .await;
    ...
    pub async fn send_dcdt_direct_multi_transfer(&mut self) {
        let to = Address::zero();
        ...
        let response = self
            ...
            .gas(80_000_000u64)
            .typed(proxy::ForwarderProxy)
            .send_dcdt_direct_multi_transfer(to, token_payments)
            .returns(ReturnsResultUnmanaged)
            .run()
            .await;
    ...
    pub async fn get_dcdt_local_roles(&mut self) {
        let token_id = DcdtTokenIdentifier::from_dcdt_bytes(&b""[..]);
    ...
    pub async fn get_dcdt_token_data(&mut self) {
    ...
    pub async fn is_dcdt_frozen(&mut self) {
        let address = Address::zero();
        let token_id = DcdtTokenIdentifier::from_dcdt_bytes(&b""[..]);
        let nonce = 0u64;
        ...
        let result_value = self
            ...
            .to(self.state.current_address())
            .typed(proxy::ForwarderProxy)
            .is_dcdt_frozen(address, token_id, nonce)
            .returns(ReturnsResultUnmanaged)
            .run()
            .await;
    ...
    pub async fn is_dcdt_paused(&mut self) {
        let token_id = DcdtTokenIdentifier::from_dcdt_bytes(&b""[..]);
    ...
    pub async fn is_dcdt_limited_transfer(&mut self) {
        let token_id = DcdtTokenIdentifier::from_dcdt_bytes(&b""[..]);
        ...
        let result_value = self
            ...
            .to(self.state.current_address())
            .typed(proxy::ForwarderProxy)
            .is_dcdt_limited_transfer(token_id)
            .returns(ReturnsResultUnmanaged)
            .run()
            .await;
    ...
    pub async fn buy_nft(&mut self) {
        ...
        let nft_id = DcdtTokenIdentifier::from_dcdt_bytes(&b""[..]);
        let nft_nonce = 0u64;
    ...
    pub async fn set_local_roles(&mut self) {
        let address = Address::zero();
        let token_identifier = DcdtTokenIdentifier::from_dcdt_bytes(&b""[..]);
        let roles = MultiValueVec::<DcdtLocalRole>::new();
    ...
    pub async fn change_to_dynamic(&mut self, token_id: &[u8]) {
        ...
        let response = self
            ...
            .gas(80_000_000u64)
            .typed(proxy::ForwarderProxy)
            .change_to_dynamic(DcdtTokenIdentifier::from(token_id))
            .returns(ReturnsResultUnmanaged)
            .run()
            .await;
    ...
    pub async fn update_token(&mut self, token_id: &[u8]) {
        let response = self
            ...
            .gas(80_000_000u64)
            .typed(proxy::ForwarderProxy)
            .update_token(DcdtTokenIdentifier::from(token_id))
            .returns(ReturnsResultUnmanaged)
            .run()
            .await;
    ...
    pub async fn modify_royalties(&mut self, token_id: &[u8], nonce: u64, new_royalty: u64) {
        ...
        let response = self
            ...
            .gas(80_000_000u64)
            .typed(proxy::ForwarderProxy)
            .modify_royalties(DcdtTokenIdentifier::from(token_id), nonce, new_royalty)
            .returns(ReturnsResultUnmanaged)
            .run()
            .await;
    ...
    pub async fn modify_creator(&mut self, token_id: &[u8], nonce: u64) {
        let response = self
            ...
            .gas(80_000_000u64)
            .typed(proxy::ForwarderProxy)
            .modify_creator(DcdtTokenIdentifier::from(token_id), nonce)
            .returns(ReturnsResultUnmanaged)
            .run()
            .await;
    ...
    pub async fn set_roles_from_wallet(
        ...
        self.interactor
            ...
            .set_special_roles(
                ...
                DcdtTokenIdentifier::from(token_id),
                roles.into_iter(),
            )
            .run()
            .await;
    ...
    pub async fn mint_nft_from_wallet<T: TopEncode>(
        ...
        self.interactor
            ...
            .dcdt_nft_create(token_id, amount, name, royalties, hash, attributes, &uris)
            .returns(ReturnsResult)
            .run()
            .await
    }

    pub async fn send_dcdt_from_wallet(
        &mut self,
        to: &Address,
        token_id: &[u8],
        nonce: u64,
        ...
        self.interactor
            ...
            .from(&self.wallet_address)
            .to(to)
            .single_dcdt(&token_id.into(), nonce, &amount.into()) // .transfer()
            .run()
            .await;
    }
}
