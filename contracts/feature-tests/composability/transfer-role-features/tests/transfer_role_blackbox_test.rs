...
const TRANSFER_ROLE_FEATURES_PATH: DrtscPath =
    DrtscPath::new("output/transfer-role-features.drtsc.json");
...
impl TransferRoleTestState {
    fn new() -> Self {
        ...
        world
            ...
            .nonce(1)
            .dcdt_balance(TRANSFER_TOKEN, 1000);
    ...
    fn check_user_and_vault_balance(&mut self) {
        self.world
            ...
            .dcdt_balance(TRANSFER_TOKEN, 800);
        self.world
            .check_account(VAULT_ADDRESS)
            .dcdt_balance(TRANSFER_TOKEN, 100);
    }
}
...
fn test_transfer_role() {
    ...
    state
        .world
        .check_account(USER_ADDRESS)
        .dcdt_balance(TRANSFER_TOKEN, 900);
