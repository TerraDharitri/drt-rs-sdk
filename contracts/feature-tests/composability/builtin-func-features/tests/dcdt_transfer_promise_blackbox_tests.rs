...
const CODE_PATH: DrtscPath = DrtscPath::new("output/builtin-func-features.drtsc.json");
...
fn transfer_fungible_promise_no_callback_blackbox_test() {
    ...
    world
        ...
        .nonce(1)
        .balance(1000)
        .dcdt_balance(FUNGIBLE_TOKEN_ID, 1000)
        .dcdt_balance(NON_FUNGIBLE_TOKEN_ID, 1000);
    ...
    world
        ...
        .dcdt_balance(FUNGIBLE_TOKEN_ID, 1000u64);

    world
        .check_account(BUILTIN_FEATURES_FUNC_ADDRESS)
        .dcdt_balance(FUNGIBLE_TOKEN_ID, 0u64);
}
