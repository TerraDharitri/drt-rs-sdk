...
const REWARDS_DISTRIBUTION_PATH: DrtscPath = DrtscPath::new("output/rewards-distribution.drtsc.json");
const SEED_NFT_MINTER_ADDRESS: TestSCAddress = TestSCAddress::new("seed-nft-minter");
const SEED_NFT_MINTER_PATH: DrtscPath =
    DrtscPath::new("../seed-nft-minter/output/seed-nft-minter.drtsc.json");
...
fn test_raffle_and_claim() {
    ...
    for nonce in nft_nonces.into_iter() {
        let payment = DcdtTokenPayment::new(NFT_TOKEN_ID.into(), nonce, 1u64.into());
...

    {
        ...
        for nft_nonce in nft_nonces {
            account_setter = account_setter.dcdt_nft_balance(NFT_TOKEN_ID, nft_nonce, 1, ());
        }
    }
