...
use ping_pong_moa::proxy_ping_pong_moa::{self, ContractState, UserStatus};

use dharitri_sc_snippets::imports::*;

const INTERACTOR_SCENARIO_TRACE_PATH: &str = "interactor_trace.scen.json";

const PING_PONG_CODE: DrtscPath = DrtscPath::new("../output/ping-pong-moa.drtsc.json");

pub async fn ping_pong_moa_cli() {
    env_logger::init();

    let config = Config::load_config();

    let mut interact = PingPongMoaInteract::init(config).await;
...
pub struct PingPongMoaInteract {
...
impl PingPongMoaInteract {
    pub async fn init(config: Config) -> Self {
        ...
        interactor.set_current_dir_from_workspace("contracts/examples/ping-pong-moa/interactor");
    ...
    pub async fn deploy(
        ...
        let (new_address, status, message) = self
            ...
            .gas(30_000_000u64)
            .typed(proxy_ping_pong_moa::PingPongProxy)
            .init(
                ping_amount,
                duration_in_seconds,
        ...
        self.state.set_ping_pong_moa_address(new_address);
    ...
    pub async fn upgrade(
        ...
        let response = self
            .interactor
            .tx()
            .to(self.state.current_ping_pong_moa_address())
    ...
    pub async fn ping(&mut self, moa_amount: u64, message: Option<&str>, sender: &Bech32Address) {
