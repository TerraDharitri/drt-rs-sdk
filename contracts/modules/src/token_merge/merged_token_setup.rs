...
pub trait MergedTokenSetupModule {
    ...
    fn add_mergeable_tokens_to_whitelist(&self, tokens: MultiValueEncoded<DcdtTokenIdentifier>) {
    ...
    fn remove_mergeable_tokens_from_whitelist(
        &self,
        tokens: MultiValueEncoded<DcdtTokenIdentifier>,
    ) {
        let mut whitelist = self.mergeable_tokens_whitelist();
        for token in tokens {
    ...
    fn create_merged_token<AttributesCreator: MergedTokenAttributesCreator<ScType = Self>>(
        &self,
        merged_token_id: DcdtTokenIdentifier,
    ...
    fn mergeable_tokens_whitelist(&self) -> UnorderedSetMapper<DcdtTokenIdentifier>;
}
