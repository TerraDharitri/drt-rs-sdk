...
pub trait UserEndpointsModule: storage::StorageModule + events::EventsModule {
            ...
            + Default,
    {
        ...
        let offered_token = &dcdt_payment.token_identifier;
        let nonce = dcdt_payment.token_nonce;
        let sell_amount = &dcdt_payment.amount;
            ...
            + Default,
    {
        ...
        match requested_nonce {
            OptionalValue::Some(nonce) => {
                self.tx()
                    .to(&caller)
                    .single_dcdt(&requested_token, nonce, &requested_amount)
                    .transfer();
    ...
    fn send_next_available_tokens(
        ...
        self.tx().to(caller).multi_dcdt(tokens_to_send).transfer();

        self.token_details(&token)
            .update(|token_ownership| token_ownership.token_nonces = nonces);
    }

    fn get_buy_price<T>(&self, amount: BigUint, identifier: DcdtTokenIdentifier) -> BigUint
    where
        T: CurveFunction<Self::Api>
            + TopEncode
            + TopDecode
    ...
    fn get_sell_price<T>(&self, amount: BigUint, identifier: DcdtTokenIdentifier) -> BigUint
    where
        T: CurveFunction<Self::Api>
            + TopEncode
            + TopDecode
    ...
    fn check_token_exists(&self, issued_token: &DcdtTokenIdentifier) {
        require!(
    ...
    fn check_owned_return_payment_token<T>(
        &self,
        issued_token: &DcdtTokenIdentifier,
        amount: &BigUint,
    ...
    fn compute_buy_price<T>(&self, identifier: &DcdtTokenIdentifier, amount: &BigUint) -> BigUint
    where
        T: CurveFunction<Self::Api>
            + TopEncode
            + TopDecode
    ...
    fn compute_sell_price<T>(&self, identifier: &DcdtTokenIdentifier, amount: &BigUint) -> BigUint
    where
        T: CurveFunction<Self::Api>
            + TopEncode
            + TopDecode
